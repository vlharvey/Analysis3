...
;
    spawn,'ls '+pth+syear0+'-'+sday+'_v04.20_r05.nc',fname
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='polar_daily_daisy_sh_'+syear+'-'+sday+'.ps'
   !p.charsize=1.5
   !p.thick=1.5
   !p.charthick=5
   !p.charthick=5
...
level=[0.01,.1,1.+findgen(nlvls-1)]
...
slevel=['0.01','0.1',strcompress(long(level(index)),/remove_all)]
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim polar_daily_daisy_sh_'+syear+'-'+sday+'.ps -rotate -90 polar_daily_daisy_sh_'+syear+'-'+sday+'.jpg'
;  spawn,'rm -f polar_daily_daisy_sh_'+syear+'-'+sday+'.ps'
endif
