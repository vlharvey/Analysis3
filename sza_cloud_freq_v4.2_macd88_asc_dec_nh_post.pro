;
; NH post-PMC season Sep 1 to 15 with the solstice on day 172
...
;
plotit:
;restore,'sza_nh_post_'+syear+'_QF01_ALB1.sav'	;,asc_freq,des_freq,dfs,szabin,syear,albmax_asc,albmin_asc,albmax_des,albmin_des
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='sza_cloud_freq_v4.2_'+syear+'_nh_post.ps'
   !p.charsize=1.5
   !p.thick=1.5
   !p.charthick=5
   !p.charthick=5
...
save,file='sza_nh_post_'+syear+'_QF01_ALB1.sav',asc_freq,des_freq,dfs,szabin,syear,albmax_asc,albmin_asc,albmax_des,albmin_des
;
; Close PostScript file and return control to X-windows
 if setplot ne 'ps' then stop
 if setplot eq 'ps' then begin
    device, /close
    spawn,'convert -trim sza_cloud_freq_v4.2_'+syear+'_nh_post.ps -rotate -90 '+$
                        'sza_cloud_freq_v4.2_'+syear+'_nh_post.jpg'
    spawn,'rm -f sza_cloud_freq_v4.2_'+syear+'_nh_post.ps'
 endif
end
