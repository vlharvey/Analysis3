...
;data = where(abs(airs_frac_day_all - scenetime/24.) le .25 and abs(lat-cips_lat_mean) lt 10. $		; more correct to use scenetime?

data = where(airs_frac_day_all*24. gt 8. and airs_frac_day_all*24. lt 9. and abs(lat-cips_lat_mean) lt 20. $				; for Island case isolate single orbit over north part of scene
;data = where(airs_frac_day_all*24. gt 10. and airs_frac_day_all*24. lt 11. and abs(lat-cips_lat_mean) lt 20. $				; for Island case isolate single orbit over south part of scene
			and (abs(lon - cips_lon_mean) lt 30.),nx)	; or $
;		abs(lon+360. - cips_lon_mean) lt 50. or $
;		abs(lon-360. - cips_lon_mean) lt 50.),nx) 		; remove GM logic for Island example
...
stimelab=strcompress(long(airs_frac_day*24.),/r)
...
;stop
			...
			if cipsplot eq 1 and airsplot eq 1 then begin
				...
				filename = 'AIRS_T_pt_+CIPS_DOY'+strtrim(string(CIPS_doy,format='(i3.3)'),2)+strmid(string(cips_frac_day,format='(f5.3)'),1,3)$
					...
					+strtrim(string(cips_lon_mean,format = '(i5.3)'),2)+'_4mu_'+stimelab+'ut.png'
			endif
			...
			IF airsplot eq 0 then begin
				...
				+'_orbit'+orbitnum+'_scene'+scenestr+'_lat'+strtrim(string(cips_lat_mean,format = '(i4.2)'),2)+'_lon'$
				+strtrim(string(cips_lon_mean,format = '(i5.3)'),2)+'_'+stimelab+'ut.png'
			endif
...
limit = [cips_lat_mean-6,cips_lon_mean-8,cips_lat_mean+6,cips_lon_mean+8]; Set plot limits based on the mean CIPS lat/lon
...
;               col2 = [250,235,220,190,170,125,125,100,80,60,30,15,10,0]
                col2 = [250,235,220,190,125,125,80,60,30,15,10,0]

			if orbitlist[iorbit] eq '49883' then level2=[-12.,-8.,-4.,-3.,-2.,-1.,0.,1.,2.,3.,4.,8.,12.]
			if orbitlist[iorbit] eq '49877' then level2=[-8.,-4.,-3.,-2.,-1.,0.,1.,2.,3.,4.,8.]
...
for nn=0L,n_elements(level2)-2L do begin
    ...
    if index(0) ne -1L then oplot,xc(index),yc(index),color=col2(nn+1),psym=8,symsize=0.25
			...
			if orbitlist[iorbit] eq '49883' or orbitlist[iorbit] eq '49877' then begin
				Map_Set, cips_lat_mean, cips_lon_mean, /Satellite, /NoErase, position = [.2,.2,.8,.8],$
				/ISOTROPIC, color=0, $
...
;        		MAP_GRID, /LABEL, LATLAB=floor(cips_lon_mean/15.)*15.-20., LONLAB=floor(cips_lat_mean/10.)*10+17, LATDEL=5, LONDEL=10
 		        MAP_GRID, /LABEL, LATLAB=floor(cips_lon_mean)+8., LONLAB=floor(cips_lat_mean)-5, LATDEL=5, LONDEL=5, charsize=2.5
;
...
;contour,sp,alon2,alat,/overplot,levels=20+2*findgen(30),color=0,c_labels=1+fltarr(30)
loadct,39
                        MAP_CONTINENTS, fill_continents=1,color = 190
                        MAP_CONTINENTS, color=0,mlinethick=5,mlinestyle=0
...
loadct,0
				...
				!p.title = ' '
				if airsplot eq 0 then position = [.3,0.15, .7,.22]
