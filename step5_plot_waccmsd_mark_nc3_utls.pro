;
; plot mercator SDWACCM O3 from nc3 files
;
@stddat
@kgmt
@ckday
...
;
        if setplot eq 'ps' then begin
           ...
           !p.font=0
           device,font_size=9
           device,/landscape,bits=8,filename='merc_sdwaccm_'+sdate+'_'+slev+'.ps'
           device,/color
...
;       level=min(pp(nhindex))+((max(pp(nhindex))-min(pp(nhindex)))/float(nlvls))*findgen(nlvls)
        level=0.025*findgen(nlvls)
        contour,pp,x,alat,levels=level,/cell_fill,c_color=col1,/noeras,xrange=[0.,360.],yrange=[0.,90],$
                charsize=1.5,color=0,xticks=4,yticks=6,charthick=2,title='SD-WACCM          '+sdate+' '+string(long(th(k)))+' K'
        contour,pp,x,alat,levels=level,/follow,color=0,/overplot
        pp2=sf
        level2=min(pp2(nhindex))+((max(pp2(nhindex))-min(pp2(nhindex)))/float(10))*findgen(10)
...
; pole values are off
        pv(*,-1)=pv(*,-2)
        pp=pv*1.e6
        contour,pp,x,alat,levels=[4],/follow,color=mcolor*.9,thick=6,/overplot,c_labels=[0]
        imin=min(level)
        imax=max(level)
        ymnb=yorig(0) -0.06
        ymxb=ymnb  +0.01
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim merc_sdwaccm_'+sdate+'_'+slev+'.ps -rotate -90 merc_sdwaccm_'+sdate+'_'+slev+'.jpg'
;      spawn,'rm -f merc_sdwaccm_'+sdate+'_'+slev+'.ps'
    endif
