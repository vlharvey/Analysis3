...
ifileb=[$
;'NOGAPS_20080701_10day_rdf_350.traj'$
;'NOGAPS_20080706_10day_rdf_350.traj'$
'NOGAPS_20080707_30day_rdf_350.traj'$
;'NOGAPS_20080708_10day_rdf_350.traj'$
]
ifilef=[$
;'NOGAPS_20080701_10day_fdf_350.traj'$
;'NOGAPS_20080706_10day_fdf_350.traj'$
'NOGAPS_20080707_30day_fdf_350.traj'$
;'NOGAPS_20080708_10day_fdf_350.traj'$
]
...
;
; eliminate soundings with no data and just take 350 K level (element 0)
;
hindex=where(thhal eq 350.,numh)
tall=thal(hindex)
xall=xhal(hindex)
...
dtout=0.
ofile='NOGAPS_20080707_30day_fdf_350.traj'
nr=0L
nc=0L
time=0.
ntraj=0l
...
;
    ...
    readu,12,char14
    READU,12,xmintnf
; Adding O3 variable                          VLH 8/11/2011
    readu,12,char15
    readu,12,o3nf
; Adding H2O variable                         VLH 8/11/2011
    readu,12,char16
    readu,12,h2onf
...
;
    ...
    if sindex(0) ne -1 then begin
       xdayf=xall(sindex)
       ydayf=yall(sindex)
       thdayf=thall(sindex)	; all data is at 350 K
       o3dayf=o3all(sindex)
...
;
   ...
   index=where(o3mapf ne 9999.,nfilled)
...
;
   ...
   index=where(o3mapb ne 9999.,nfilled)
...
for k=0,nthp-1 do begin
    if agemapf(ncount) lt agemapb(ncount) and $
       o3mapf(ncount) ne 9999. then begin
    ...
    if agemapb(ncount) lt agemapf(ncount) and $
       o3mapb(ncount) ne 9999. then begin
...
index=where(o3map ne 9999.,nfilled)
if index(0) ne -1 then print,'Ozone ',max(o3map(index)),$
   min(o3map(index)),float(nfilled)*100./ntraj,' % final filled'
;
