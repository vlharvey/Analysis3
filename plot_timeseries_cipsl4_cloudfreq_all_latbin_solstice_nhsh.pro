...
;
    if setplot eq 'ps' then begin
       ...
       device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
              /bold,/color,bits_per_pixel=8,/helvetica,filename='timeseries_cipsl4_cloudfreq_all_lat.ps'
       !p.charsize=1.25
       !p.thick=2
       !p.charthick=5
       !p.charthick=5
...
;
    ...
    plot,findgen(ndoy),cloud_freq_max_lat2,ytitle='Cloud Frequency (%)',xrange=[imin,imax],$
         xticks=nxticks,xtickname=xlab,xtickv=float(xlab),charsize=1.5,color=0,/nodata,$
         title='CIPS Level 4 V3.20',yrange=[0.,100.]
    endif
    ...
    if index2(0) ne -1L then oplot,fdoy_all(index2)-nhsol,cloud_freq_avg_lat2(index2),color=icol,psym=1,symsize=2
    if index2(0) ne -1L then oplot,fdoy_all(index2)-nhsol,cloud_freq_avg_lat2(index2),color=icol,psym=0,thick=3
    xyouts,-38.,90.,'2007 (dot)',color=0,/data,charsize=1.5,charthick=5
    xyouts,-38.,82.5,'2008 (plus)',color=0,/data,charsize=1.5,charthick=5
    if abs(latbin(ilat2)) ge 60. then xyouts,70.,latbin(ilat2)+5.,slat+'N',color=icol,/data,charsize=1.5,charthick=5
skiplatnh:
endfor
...
;
    ...
    plot,findgen(ndoy),cloud_freq_max_lat,ytitle='Cloud Frequency (%)',xrange=[imin,imax],$
         xticks=nxticks,xtickname=xlab,xtickv=float(xlab),charsize=1.5,color=0,/nodata,$
         yrange=[0.,100.],xtitle='Days from Solstice'
    endif
    syear=strmid(strcompress(sdate_all,/remove_all),0,4)
;
    shsol=355.
...
print,cloud_freq_avg_lat(index1)
    if index1(0) ne -1L then oplot,fdoy_all(index1)-365.,cloud_freq_avg_lat(index1),color=icol,psym=1,symsize=2
    if index1(0) ne -1L then oplot,fdoy_all(index1)-365.,cloud_freq_avg_lat(index1),color=icol,psym=0,thick=3
    index1=where(syear eq '2009' and fdoy_all lt shsol+80. and cloud_freq_avg_lat ge 0.1)
print,fdoy_all(index1)
    if index1(0) ne -1L then oplot,fdoy_all(index1),cloud_freq_avg_lat(index1),color=icol,psym=1,symsize=2
    if index1(0) ne -1L then oplot,fdoy_all(index1),cloud_freq_avg_lat(index1),color=icol,psym=0,thick=3
...
if ilat eq 0 then stop
    ...
    xyouts,-38.,90.,'2007-2008 (dot)',color=0,/data,charsize=1.5,charthick=5
    xyouts,-38.,82.5,'2008-2009 (plus)',color=0,/data,charsize=1.5,charthick=5
    if latbin(ilat) le -60. then xyouts,70.,abs(latbin(ilat))+5.,slat+'S',color=icol,/data,charsize=1.5,charthick=5
skiplatsh:
endfor
...
;
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim timeseries_cipsl4_cloudfreq_all_lat.ps -rotate -90 timeseries_cipsl4_cloudfreq_all_lat.jpg'
    endif
