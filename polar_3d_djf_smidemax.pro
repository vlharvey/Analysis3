;
; WACCM 3D Arctic vortex and anticyclones for 300-year DJF average
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='polar_3d_djf_smidemax.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !p.charthick=5
...
;
	...
	if lindex(0) ne -1 then begin
           ...
           contour,dum,xcn,ycn,levels=[0.1],color=0,/noeras,$
...
; anticyclones
;
        index=where(y2d le 45.)         ; ignore tropical/subtropical highs
        dum(index)=0./0.
        lindex=where(dum lt -0.25,nl)
        if lindex(0) ne -1 then begin
           oplot,xcn(lindex),ycn(lindex),/T3D,zvalue=nz,psym=8,symsize=2,color=0
loadct,0
            contour,dum,xcn,ycn,levels=[-0.25],color=mcolor*.7,$
...
;
    !psym=0
    xyouts,0.42,0.85,'DJF',/normal,charsize=3,color=0,charthick=2
...
col1=1+(findgen(nlvls)/nlvls)*mcolor
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim polar_3d_djf_smidemax.ps -rotate -90 polar_3d_djf_smidemax.jpg'
;      spawn,'rm -f polar_3d_djf_smidemax.ps'
    endif
end
