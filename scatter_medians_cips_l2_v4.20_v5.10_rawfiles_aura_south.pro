...
erase
      if setplot eq 'ps' then begin
         ...
         device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
                /bold,/color,bits_per_pixel=8,/helvetica,filename='scatter-6pan_cips_l4_v4.20_v5.10_rawfiles_south_'+syear+'.ps'
         !p.charsize=1.25
         !p.thick=2
         !p.charthick=5
         !p.charthick=5
...
if index(0) ne -1L then oplot,medalb20_all(index),medalb51_all(index),color=90,psym=8,symsize=1
...
if index(0) ne -1L then oplot,mediwc20_all(index),mediwc51_all(index),color=90,psym=8,symsize=1
...
if index(0) ne -1L then oplot,medrad20_all(index),medrad51_all(index),color=90,psym=8,symsize=1
oplot,level,level,thick=2,color=0
...
oplot,level,level,thick=2,color=0
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim scatter-6pan_cips_l4_v4.20_v5.10_rawfiles_south_'+syear+'.ps -rotate -90 scatter-6pan_cips_l4_v4.20_v5.10_rawfiles_south_'+syear+'.jpg'
;      spawn,'rm -f scatter-6pan_cips_l4_v4.20_v5.10_rawfiles_south_'+syear+'.ps'
    endif
skipyear:
endfor
end
