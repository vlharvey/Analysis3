c***************************************************************************c
c                                                                           c
c  Program: ATTREX_traject_3d_v12                                            c
...
c  Input parameter file: ATTREX_traject_3d_v13.fil                           c
...
c                                              VLH 8/2004
c
c Version 21 updates:
c        read ATTREX flight data
c						VLH 6/7/2013
c***************************************************************************c
      program ATTREX_traject_3d_v21
c
      include "netcdf.inc"
c
      include 'comm_ukmo_3d_v21.'
c
...
c
      print *, 'Opening ATTREX_traject_3d_v21.fil'
      OPEN(5,FILE='ATTREX_traject_3d_v21.fil',status='old',READONLY)
...
c initialize ATTREX flight path
        if (attrex) then
          if (dt .gt. 0.0) then           ! forward trajectories
            do id = 1, ncount
              idial = id
              call init_attrex(idial)
c             write(6,340) idial, ntraj
            end do
          endif
          if (dt .lt. 0.0) then           ! back trajectories
            do id = ncount,1,-1
...
c  The initial DC-8 data includes dynamic characteristics  VLH 9/10/2002
        if (.not. dial .and. .not. dc8 .and. .not. attrex) call getdiag
c adjust etime up for call to calendar         Duncan 5/12/2000 
        rtime = etime + 0.0001
        call calendar(istime,rtime,ictime)
...
c  introduce any new DIAL, DC-8, or ATTREX profiles at this timestep   Duncan 6/28/99 
...
c adjust etime up for call to calendar         Duncan 5/12/2000 
            ...
            if (attrex) itdial_date=attrex_date(idial)
c           if (attrex) print *,'ATTREX DATE ',itdial_date
            if (dt .lt. 0.0) then    ! back trajectory condition 
   21         continue 
              if (idial.ge.1) then
                tdial = tsave(idial) 
                if (attrex) tdial=atttime(idial)
...
c               write(6,359) ictime, date, itdial_date 
     ...
     1             .and. (tdiff .lt. abs(dtb2))) then
                  ...
                  if (attrex) call init_attrex(idial)
                  idial=idial-1
                  goto 21
                endif
              endif
...
c               write(6,359) ictime, date, itdial_date 
  ...
  360 format (' # ',I6,' tdiff ',F10.4,' model gmt ',
     1         F10.4,' dial time ',F10.4,' ntraj =',I6)
  361 format (' idial',I6,' istime = ',I10,' etime = ',F10.4,
     1        ' ictime = ',I10)
C
...
c
c  read in ATTREX data for the start day           VLH 6/7/2013
      if (attrex) call read_attrex
c
...
c what is 0 < theta < 2*pi for igwave routine? DF 10/16/97
                  call igwave(uigw,vigw,u,v,i,j,xp,yp,zp,wmag,theta) 
                  u=u+uigw
                  v=v+vigw
                endif
              endif
...
c
      ...
      do nf=1,119
         if(test(nf:nf+1).ne.' ')len=len+1
         if(diratt(nf:nf+1).ne.' ')lend=lend+1
      enddo

      print *,'ENTERED READ ATTREX'

c We need the ATTREX date for ATTREX initialisation
c

      status=nf_open(diratt(1:lend)//test(1:len),nf_nowrite,ncid)
      print *,'opening ',diratt(1:lend)//test(1:len)
c get dimensions
c
...
c     status=nf_get_vara_real(ncid,1,1,nt,attrex_date)
c     status=nf_get_vara_real(ncid,1,1,nt,atttime)
      status=nf_get_vara_real(ncid,1,1,nt,attlon)
      status=nf_get_vara_real(ncid,2,1,nt,attlat)
      status=nf_get_vara_real(ncid,3,1,nt,atttheta)
      status=nf_get_vara_real(ncid,4,1,nt,attPV)
      status=nf_get_vara_real(ncid,5,1,nt,attP)
      status=nf_get_vara_real(ncid,6,1,nt,attU)
      status=nf_get_vara_real(ncid,7,1,nt,attV)
      status=nf_get_vara_real(ncid,8,1,nt,attQ)
      status=nf_get_vara_real(ncid,9,1,nt,attQDF)
      status=nf_get_vara_real(ncid,10,1,nt,attMark)
      status=nf_get_vara_real(ncid,11,1,nt,attGeos_Theta)
      status=nf_get_vara_real(ncid,12,1,nt,attTemp)
      status=nf_get_vara_real(ncid,13,1,nt,attAlt)

      do i=1,ntamx 
         atttime(i)=12.
         attrex_date(i)=istime/100
      enddo

c print variables to check that they are ok

      print *,'ATTREX TIME ',atttime(1)
      print *,'ATTREX DATE ',attrex_date(1)
      print *,'Lon ',attLon(200:300)
      print *,'Lat ',attLat(200:300)
      print *,'Theta ',attTheta(200:300)
      print *,'PV ',attPV(200:300)
      ...
      print *,' ATTREX file read OK, ncount =',ncount
      return
      end
...
c Adding t0n and agen arrays                       Duncan 11/05/99
      ...
      subroutine init_attrex(idial)
c
c*****************************************************************c
c                                                                 c
c   Initialise ATTREX flight at point # idial                     c
c                                                                 c
c*****************************************************************c
c
      include 'comm_ukmo_3d_v21.'
c
c initialize x,y,th coincident with time
c
      ist = ntraj+1
      n = ist
      xn(n)=attlon(idial)
