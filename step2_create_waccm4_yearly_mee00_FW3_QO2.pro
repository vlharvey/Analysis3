...
spawn,'ls '+dirh+'*0101_QO2.sav',ifiles
...
; read WACCM data
;
      dum=findfile(dirh+'Year'+model_years(iyear)+'_'+sdate+'_QO2.sav')
...
;
      if icount eq 0L then begin
         QRS_SO2A00FPL_avg=fltarr(nr,nl,kday)
         QRS_SO2B00FPL_avg=fltarr(nr,nl,kday)
      endif
;
; loop over years and retain all data
;
      print,'restored '+dum(0)
      QRS_SO2A00FPL_avg(*,*,icount)=QRS_SO2A00FPL
      QRS_SO2B00FPL_avg(*,*,icount)=QRS_SO2B00FPL
;
jumpday:
      icount=icount+1L
goto,jump
...
;
saveit:
ofile=dirh+'Year'+model_years(iyear)+'_VLH_QO2.sav'
save,file=ofile,sdate_all,lat,lev,QRS_SO2A00FPL_avg,QRS_SO2B00FPL_avg
