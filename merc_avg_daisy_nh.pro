...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='merc_avg_daisy_nh_'+syear+'.ps'
   !p.charsize=1.5
   !p.thick=1.5
   !p.charthick=5
   !p.charthick=5
...
map_set,0,0,0,/contin,/grid,color=0,/noeras,title='CIPS L3 V4.20 on -10 to +30 DFS in '+syear,limit=[50, 0, 80, 360]
level=[.1,1.+findgen(nlvls-1)]
bad=where(finite(alb_avg) ne 1 or alb_avg lt 0.)
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim merc_avg_daisy_nh_'+syear+'.ps -rotate -90 merc_avg_daisy_nh_'+syear+'.jpg'
;  spawn,'rm -f merc_avg_daisy_nh_'+syear+'.ps'
endif
end
