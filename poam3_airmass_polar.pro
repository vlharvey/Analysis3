...
for n=1,nstep-1 do begin
    ...
    ofile='poam3_'+stimes(n)+'_'+stheta
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename=ofile+'+diff.ps'
       device,/color
       device,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,$
              xsize=xsize,ysize=ysize
    endif
    xyouts,.25,.9,'POAM III Trajectories + Diffusion',/normal,charsize=2
    MAP_SET,90,0,-90,/ortho,/noeras,charsize=1.5,$
            title='Valid '+stimes(n)+'  ('+stheta+')'
    iflag=0
    idy=strmid(stimes(n),6,2)
    imn=strmid(stimes(n),4,2)
    iyr=strmid(stimes(n),2,2)
    ufile=dirw+mon(imn-1)+(string(FORMAT='(I2.2,A1,I2.2)',idy,'_',iyr))+'.nc3'
    print,ufile
    rd_ukmo_nc3,ufile,nc,nr,nl,alon,alat,th,pv2,p2,msf2,$
             u2,v2,q2,qdf2,mark2,vp2,sf2,iflag
    if iflag eq 1 then goto,skipstep
    ...
    index=where(xpos ne -9999. and agepos le 1000.,nleg)
    xpos=xpos(index)
    ypos=ypos(index)
    x0pos=x0pos(index)
    y0pos=y0pos(index)
...
wait,.5
     if setplot eq 'ps' then begin
        device, /close
        spawn,'convert '+ofile+'+diff.ps -rotate -90 '+ofile+'+diff.jpg'
     endif
     skipstep:
endfor
jumprun:
