...
nh=1
ifilet=['../WACCM_20060118_10day_fdf_MIL.traj']
...
ilat=60.
slat=strcompress(long(ilat),/remove_all)
yindex=where(abs(alat-ilat) eq min(abs(alat-ilat)))
    qbar=fltarr(nc,inth)
    tbar=fltarr(nc,inth)
    zbar=fltarr(nc,inth)
    vbar=fltarr(nc,inth)
    ...
    for j=0L,nc-1L do begin
        qbar(j,k)=reform(q2(yindex(0),j,k))
        tbar(j,k)=reform(tmp2(yindex(0),j,k))
        zbar(j,k)=reform(gph2(yindex(0),j,k))
        vbar(j,k)=reform(v2(yindex(0),j,k))
        ubar(j,k)=reform(u2(yindex(0),j,k))
        mbar(j,k)=reform(mark2(yindex(0),j,k))
    endfor
    endfor
    level=-20.+2.*findgen(nlvls)
    level=-100.+10.*findgen(nlvls)
    level=110.+10.*findgen(nlvls)
    contour,tbar,alon,zbar,/fill,c_color=col1,levels=level,xrange=[0.,360.],yrange=[60.,90.],ytitle='Geopotential Height (km)',xtitle='Longitude',title=slat
    index=where(level lt 0.)
    contour,tbar,alon,zbar,/follow,color=mcolor,c_linestyle=1,levels=level(index),/overplot
    index=where(level gt 0.)
    contour,tbar,alon,zbar,/follow,color=0,c_linestyle=0,levels=level(index),/overplot
    xx0=reform(xsave(0,*))
    yy0=reform(ysave(0,*))
    zz0=reform(gphsave(0,*))
    aa0=reform(agesave(0,*))
    tmp0=reform(tmpsave(0,*))
    mrk0=reform(xmrksfsave(0,*))
    coinindex=where(tmp0 lt 170. and xx0 gt 130. and xx0 lt 250. and yy0 ge 50. and yy0 le 70.,ncoin)	; anticyclones
    oplot,xx0(coinindex),zz0(coinindex),psym=8,color=0,symsize=2
    for nn=1,nfiles-2 do begin
        print,mdates(nn)
        xx0=reform(xsave(nn,*))
        zz0=reform(gphsave(nn,*))
        aa0=reform(agesave(nn,*))
        xx1=reform(xsave(nn+1,*))
        zz1=reform(gphsave(nn+1,*))
        aa1=reform(agesave(nn+1,*))
        for icoin=0L,ncoin-1,20 do begin
;print,icoin,yy0(coinindex(icoin)),yy1(coinindex(icoin))
if xx0(coinindex(icoin)) gt 100. and xx0(coinindex(icoin)) lt 300. then begin
            oplot,[xx0(coinindex(icoin)),xx1(coinindex(icoin))],[zz0(coinindex(icoin)),zz1(coinindex(icoin))],psym=0,thick=2,color=(abs(aa0(coinindex(icoin)))/abs(max(agesave)))*mcolor
            oplot,[xx0(coinindex(icoin)),xx0(coinindex(icoin))],[zz0(coinindex(icoin)),zz0(coinindex(icoin))],psym=8,color=(abs(aa0(coinindex(icoin)))/abs(max(agesave)))*mcolor
            oplot,[xx1(coinindex(icoin)),xx1(coinindex(icoin))],[zz1(coinindex(icoin)),zz1(coinindex(icoin))],psym=8,color=(abs(aa0(coinindex(icoin)))/abs(max(agesave)))*mcolor
endif
        endfor
    endfor
;    xx0=reform(xsave(0,*))
...
;    zz0=reform(gphsave(0,*))
;    aa0=reform(agesave(0,*))
;    tmp0=reform(xmrksfsave(0,*))
;    coinindex=where(tmp0 gt 0. and yy0 ge 50.,ncoin)    ; vortex
;    oplot,xx0(coinindex),zz0(coinindex),psym=8,color=mcolor,symsize=2
;    for nn=1,nfiles-2 do begin
;        print,mdates(nn)
;        xx0=reform(xsave(nn,*))
;        zz0=reform(gphsave(nn,*))
...
;        zz1=reform(gphsave(nn+1,*))
;        aa1=reform(agesave(nn+1,*))
;        for icoin=0L,ncoin-1,200 do begin
;;print,icoin,yy0(coinindex(icoin)),yy1(coinindex(icoin))
;            oplot,[xx0(coinindex(icoin)),xx1(coinindex(icoin))],[zz0(coinindex(icoin)),zz1(coinindex(icoin))],psym=0,thick=2,color=(abs(aa0(coinindex(icoin)))/abs(max(agesave)))*mcolor
;            oplot,[xx0(coinindex(icoin)),xx0(coinindex(icoin))],[zz0(coinindex(icoin)),zz0(coinindex(icoin))],psym=8,color=(abs(aa0(coinindex(icoin)))/abs(max(agesave)))*mcolor
;            oplot,[xx1(coinindex(icoin)),xx1(coinindex(icoin))],[zz1(coinindex(icoin)),zz1(coinindex(icoin))],psym=8,color=(abs(aa0(coinindex(icoin)))/abs(max(agesave)))*mcolor
;        endfor
;    endfor
    contour,mbar,alon,zbar,/overplot,/follow,color=mcolor,levels=[0.1],thick=15,c_linestyle=2,c_charthick=5,c_charsize=2
    contour,mbar,alon,zbar,/overplot,/follow,color=0,levels=[-0.1],thick=15,c_linestyle=2,c_charthick=5,c_charsize=2
    xx0=reform(xsave(0,*))
    yy0=reform(ysave(0,*))
    zz0=reform(gphsave(0,*))
    tmp0=reform(tmpsave(0,*))
    mrk0=reform(xmrksfsave(0,*))
    coinindex=where(tmp0 lt 170. and xx0 gt 100. and xx0 lt 300. and yy0 ge 50. and yy0 le 70.,ncoin) ; anticyclones
    oplot,xx0(coinindex),zz0(coinindex),psym=8,color=0
;    coinindex=where(tmp0 gt 0. and yy0 ge 50. and yy0 le 70.,ncoin)    ; vortex initial location
;    oplot,xx0(coinindex),zz0(coinindex),psym=8,color=mcolor,symsize=2
;
; color bar
;
kmin=0
kmax=abs(max(agesave))
ymnb=ymn -cbaryoff
ymxb=ymnb+cbarydel
