;
; run on MacD88
; read CIPS level 2a version 4.01 data and plot albedo orbit by orbit. x-axis is ice water content
;
loadct,39
mcolor=!p.color
...
;
          good=WHERE(FINITE(ALB) EQ 1,ngood)	
...
if iorbit eq 0L then begin
   if setplot eq 'ps' then begin
      ...
      device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
             /bold,/color,bits_per_pixel=8,/helvetica,filename='plot_orbit_cipsl2_alb_vs_iwc_v4.1_'+sdate+'.ps'
      !p.charsize=1
      !p.thick=1.5
      !p.charthick=5
      !p.charthick=5
   ...
   erase
   !type=2^2+2^3
   xyouts,.25,.925,'CIPS Version 4.10  '+sdate,color=0,/normal,charsize=1.75
   xyouts,.15,.06,'GOOD = FINITE(ALB) EQ 1',/normal,color=0,charsize=1.25
   xyouts,.15,.03,'CLOUD = FINITE(ALB) EQ 1 AND ALB GT 0',/normal,color=mcolor*.9,charsize=1.25
endif
xmn=xorig(iorbit)
...
ylab=' '
if iorbit eq 0 or iorbit eq 5 or iorbit eq 10 then ylab='Albedo'
plot,iwc,alb,psym=3,color=0,xtitle='IWC',ytitle=ylab,title='Orbit '+sorbit,xrange=[0.,2000.],yrange=[0.,100.]
...
if cloud(0) ne -1L then oplot,iwc(cloud),alb(cloud),psym=3,color=mcolor*.9
print,'IWC bounds ',min(iwc),max(iwc)
;print,'NCLOUD/NGOOD ',100.*float(ncloud)/float(ngood)
;print,'% CLOUDS ',percent_clouds
;xyouts,42.,90.,'NGOOD '+strcompress(long(ngood),/remove_all),/data,color=0,charsize=0.8
;xyouts,42.,80.,'NCLD '+strcompress(long(ncloud),/remove_all),/data,color=mcolor*.9,charsize=0.8
;spercent=string(FORMAT='(f4.1)',100.*float(ncloud)/float(ngood))
;spercent_clouds=string(FORMAT='(f4.1)',percent_clouds)
;xyouts,42.,70.,'NC/NG % '+spercent,/data,color=0
;xyouts,42.,60.,'CIPS % '+spercent_clouds,/data,color=0
...
; Close PostScript file and return control to X-windows
     ...
     if setplot eq 'ps' then begin
        device, /close
        spawn,'convert -trim plot_orbit_cipsl2_alb_vs_iwc_v4.1_'+sdate+'.ps -rotate -90 '+$
                            'plot_orbit_cipsl2_alb_vs_iwc_v4.1_'+sdate+'.jpg'
        spawn,'/usr/bin/rm plot_orbit_cipsl2_alb_vs_iwc_v4.1_'+sdate+'.ps'
     endif
