...
if imon gt 4 and imon lt 8 then goto,skipmon
dum=findfile('ztd+vortex_shape_sdwaccm_'+syear(iyear)+smon(imon)+'.sav')
;if dum(0) ne '' then goto,skipmon
ifiles=file_search(dirw+'f_1975-2010_2deg_refc1sd_wa4_tsmlt.002.cam.h5.'+syear(iyear)+smon(imon)+'??.nc3',count=nfile)
...
;       u2,v2,qdf2,mark2,qv2,z2,sf2,q2,iflag
    if iflg ne 0L then goto,jumpstep
    tmp2=0.*p2
...
ztd=smooth(ztd,3,/nan)
save,filename='vortex_shape_sdwaccm_'+syear(iyear)+smon(imon)+'.sav',nth,nfile,th,sdate_all,centroid_longitude1,area1,$
