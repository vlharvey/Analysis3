;
; Altitude-time section showing the minimum temperature experienced by
...
;
; loop over output times and extract minimum temperature in the vortex 
; over the past 10 days
;
tmin=9999.+fltarr(nfiles,nth)
ntmin=lonarr(nfiles,nth)
...
;
index=where(markday gt 0. and abs(ageday) le 10. and mintday ne 99999.0,nt)
if index(0) ne -1L then begin
   if min(mintday(index)) lt tmin(itime(0),klev(0)) then begin
   tmin(itime(0),klev(0))=min(mintday(index))
   ntmin(itime(0),klev(0))=ntmin(itime(0),klev(0))+1L
   endif
endif
...
;
index=where(ntmin ge 1L)
;if index(0) ne -1L then tmin(index)=tmin(index)/float(ntmin(index))
;
; plot
;
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='ace_airmass_zt_mintemp_'+stime0+'-'+stime1+'.ps'
   device,/color
...
level=180.+5.*findgen(11)
nlvls=n_elements(level)
col1=5+indgen(nlvls)*mcolor/nlvls
tmin=smooth(tmin,3,/edge_truncate)
;
; reverse date and tmin arrays to plot forward in time
;
ctimes=reverse(ctimes)
rtmin=0.*tmin
for ith=0L,nth-1L do rtmin(*,ith)=reverse(tmin(*,ith))
...
index=where(rtmin eq 9999.)
if index(0) ne -1L then rtmin(index)=-9999.
contour,rtmin,findgen(nfiles),thlev,levels=level,/noeras,/cell_fill,c_color=col1,$
    min_value=0.,c_charsize=2,color=lc,yrange=[min(thlev),700.],$
    title='ACE Min Vortex Temperature in Previous 10 days',charsize=2,$
    xtickv=xindex,xticks=nxticks-1L,xtickname=' '+strarr(nxticks)
contour,rtmin,findgen(nfiles),thlev,/overplot,levels=level,/noeras,color=0
contour,rtmin,findgen(nfiles),thlev,/overplot,levels=[188.,195],/noeras,color=mcolor,thick=3
...
!type=2^2+2^3+2^6
plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],xtitle='(K)',charsize=2
ybox=[0,10,10,0,0]
x1=imin
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim ace_airmass_zt_mintemp_'+stime0+'-'+stime1+'.ps -rotate -90 '+$
         ' ace_airmass_zt_mintemp_'+stime0+'-'+stime1+'.jpg'
endif
