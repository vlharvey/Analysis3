...
; color by nlayers
; run on MacD88
; read CIPS level 2a version 4.01 data and plot radius vs SZA each day
;
loadct,39
mcolor=!p.color
...
if iorbit eq 0L then begin
   if setplot eq 'ps' then begin
      ...
      device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
             /bold,/color,bits_per_pixel=8,/helvetica,filename='plot_daily_cipsl2_rad_vs_sza_colnlayer_v4.1_'+sdate+'.ps'
      !p.charsize=1.5
      !p.thick=1.5
      !p.charthick=5
      !p.charthick=5
...
plot,sza,rad,psym=3,color=0,xtitle='SZA',ytitle='Version 4.10 CIPS Radius (nm)',title=sdate,xrange=[40.,90.],yrange=[0.,100.]
xyouts,41.,95.,'Radius Offset by 0.06 Each Orbit',color=0,/data,charsize=1.25
endif
...
offset=0.06*iorbit
if cloud(0) ne -1L then oplot,sza(cloud),rad(cloud)+offset,psym=3,color=0
;
; color by nlayers
;
index=where(nlayers eq 0.)
if index(0) ne -1L then oplot,sza(index),rad(index)+offset,psym=3,color=mcolor*.1
index=where(nlayers eq 1.)
if index(0) ne -1L then oplot,sza(index),rad(index)+offset,psym=3,color=mcolor*.2
index=where(nlayers eq 2.)
if index(0) ne -1L then oplot,sza(index),rad(index)+offset,psym=3,color=mcolor*.3
index=where(nlayers eq 3.)
if index(0) ne -1L then oplot,sza(index),rad(index)+offset,psym=3,color=mcolor*.4
index=where(nlayers eq 4.)
if index(0) ne -1L then oplot,sza(index),rad(index)+offset,psym=3,color=mcolor*.5
index=where(nlayers eq 5.)
if index(0) ne -1L then oplot,sza(index),rad(index)+offset,psym=3,color=mcolor*.6
index=where(nlayers eq 6.)
if index(0) ne -1L then oplot,sza(index),rad(index)+offset,psym=3,color=mcolor*.7
index=where(nlayers eq 7.)
if index(0) ne -1L then oplot,sza(index),rad(index)+offset,psym=3,color=mcolor*.8
index=where(nlayers ge 8.)
if index(0) ne -1L then oplot,sza(index),rad(index)+offset,psym=3,color=mcolor*.9

endfor  ; loop over orbits

slevel=['0','1','2','3','4','5','6','7','>8']
...
; Close PostScript file and return control to X-windows
     ...
     if setplot eq 'ps' then begin
        device, /close
        spawn,'convert -trim plot_daily_cipsl2_rad_vs_sza_colnlayer_v4.1_'+sdate+'.ps -rotate -90 '+$
                            'plot_daily_cipsl2_rad_vs_sza_colnlayer_v4.1_'+sdate+'.jpg'
        spawn,'rm -f plot_daily_cipsl2_rad_vs_sza_colnlayer_v4.1_'+sdate+'.ps'
     endif
