...
; color by IWC
; run on MacD88
; read CIPS level 2a version 4.01 data and plot albedo vs SZA each day
;
loadct,39
mcolor=!p.color
...
if iorbit eq 0L then begin
   if setplot eq 'ps' then begin
      ...
      device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
             /bold,/color,bits_per_pixel=8,/helvetica,filename='plot_daily_cipsl2_alb_vs_rad_colsza_v4.1_'+sdate+'.ps'
      !p.charsize=1.5
      !p.thick=1.5
      !p.charthick=5
      !p.charthick=5
...
plot,rad,alb,psym=3,color=0,xtitle='Radius (nm)',ytitle='Version 4.10 CIPS Albedo (10!u-6!n str!u-1!n)',title=sdate,xrange=[0.,100.],yrange=[0.,100.],/nodata
xyouts,5.,95.,'Radius Offset by 0.06 Each Orbit',color=0,/data,charsize=1.25
endif
...
offset=iorbit*.06
if cloud(0) ne -1L then oplot,rad(cloud)+offset,alb(cloud),psym=3,color=0
;
; color by SZA
;
;index=where(alb gt alblim and sza ge 40. and sza le 50.)
;if index(0) ne -1L then oplot,rad(index)+offset,alb(index),psym=3,color=mcolor*.1
index=where(alb gt alblim and sza gt 50. and sza le 55.)
if index(0) ne -1L then oplot,rad(index)+offset,alb(index),psym=3,color=mcolor*.1
index=where(alb gt alblim and sza gt 55. and sza le 60.)
if index(0) ne -1L then oplot,rad(index)+offset,alb(index),psym=3,color=mcolor*.25
index=where(alb gt alblim and sza gt 60. and sza le 65.)
if index(0) ne -1L then oplot,rad(index)+offset,alb(index),psym=3,color=mcolor*.4
index=where(alb gt alblim and sza gt 65. and sza le 70.)
if index(0) ne -1L then oplot,rad(index)+offset,alb(index),psym=3,color=mcolor*.5
index=where(alb gt alblim and sza gt 70. and sza le 75.)
if index(0) ne -1L then oplot,rad(index)+offset,alb(index),psym=3,color=mcolor*.6
index=where(alb gt alblim and sza gt 75. and sza le 80.)
if index(0) ne -1L then oplot,rad(index)+offset,alb(index),psym=3,color=mcolor*.7
index=where(alb gt alblim and sza gt 80. and sza le 85.)
if index(0) ne -1L then oplot,rad(index)+offset,alb(index),psym=3,color=mcolor*.8
index=where(alb gt alblim and sza gt 85. and sza lt szalim_hi)
if index(0) ne -1L then oplot,rad(index)+offset,alb(index),psym=3,color=mcolor*.9

endfor  ; loop over orbits

slevel=['50','55','60','65','70','75','80','85','91']
...
; Close PostScript file and return control to X-windows
     ...
     if setplot eq 'ps' then begin
        device, /close
        spawn,'convert -trim plot_daily_cipsl2_alb_vs_rad_colsza_v4.1_'+sdate+'.ps -rotate -90 '+$
                            'plot_daily_cipsl2_alb_vs_rad_colsza_v4.1_'+sdate+'.jpg'
        spawn,'rm -f plot_daily_cipsl2_alb_vs_rad_colsza_v4.1_'+sdate+'.ps'
     endif
