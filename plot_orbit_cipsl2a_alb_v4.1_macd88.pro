...
if iorbit eq 0L then begin
   if setplot eq 'ps' then begin
      ...
      device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
             /bold,/color,bits_per_pixel=8,/helvetica,filename='plot_orbit_cipsl2_alb_v4.1_'+sdate+'_'+sorbit+'.ps'
      !p.charsize=1.25
      !p.thick=2
      !p.charthick=5
      !p.charthick=5
   ...
   erase
   !type=2^2+2^3
   xyouts,.3,.95,'CIPS Version 4.10  '+sdate,color=0,/normal,charsize=2
endif
xmn=xorig(iorbit)
...
;          xtitle='Version 4.10 CIPS Albedo (10!u-6!n str!u-1!n)'
;    ybox=[0,10,10,0,0]
;    x2=imin
...
;     if setplot ne 'ps' then stop
     if setplot eq 'ps' then begin
        device, /close
        spawn,'convert -trim plot_orbit_cipsl2_alb_v4.1_'+sdate+'.ps -rotate -90 '+$
                            'plot_orbit_cipsl2_alb_v4.1_'+sdate+'.jpg'
;       spawn,'/usr/bin/rm plot_orbit_cipsl2_alb_v4.1_'+sdate+'.ps'
     endif
