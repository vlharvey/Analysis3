;
; MacD88 version
; read CIPS level 2a version 4.10 data and plot albedo for all orbits each day
;
loadct,39
mcolor=!p.color
...
pth='/Volumes/earth/harvey/CIPS_data/Datfiles/cips_sci_2_orbit_'
...
;
          good=WHERE(FINITE(ALB) EQ 1 and SZA LT SZALIM_HI AND SZA GT SZALIM_LO,ngood)	; AND FINITE(ALB_ERR) EQ 1 AND $
...
;
    if setplot eq 'ps' then begin
       ...
       device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
              /bold,/color,bits_per_pixel=8,/helvetica,filename='polar_daily_cipsl2_alb_v4.1_'+sdate+'.ps'
       !p.charsize=1.25
       !p.thick=2
       !p.charthick=5
       !p.charthick=5
   ...
   !type=2^2+2^3
   map_set,90,0,-90,/ortho,/contin,/grid,title=sdate,color=0,/usa,limit=[30.,0.,90.,360.]
endif
amin=0.
amax=60.
...
; Close PostScript file and return control to X-windows
     ...
     if setplot eq 'ps' then begin
        device, /close
        spawn,'convert -trim polar_daily_cipsl2_alb_v4.1_'+sdate+'.ps -rotate -90 '+$
                            'polar_daily_cipsl2_alb_v4.1_'+sdate+'.jpg'
        spawn,'rm -f polar_daily_cipsl2_alb_v4.1_'+sdate+'.ps'
     endif
