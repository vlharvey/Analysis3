...
; restore,'/Volumes/cloud/data/WACCM_data/Datfiles_SD_New/ f.e11.FWTREFC1SD.f19.f19.ccmi30.001.cam.h5.20110824.sav
; LAT             DOUBLE    = Array[96]
; LEV             DOUBLE    = Array[88]
...
ifiles=file_search(dirw+'f.e11.FWTREFC1SD.f19.f19.ccmi30.001.cam.h5.*.sav',count=nfile)
;
; loop over files
;
...
co2GRD=fltarr(nr,nc,nth)
noGRD=fltarr(nr,nc,nth)
...
; LINEARLY INTERPOLATE IN THETA TO ISENTROPIC LEVEL
      ...
      co2GRD(LAT,I,M)=co2(I,LAT,K)+SCALE*(co2(I,LAT,LM1)-co2(I,LAT,K))
      noGRD(LAT,I,M)=no(I,LAT,K)+SCALE*(no(I,LAT,LM1)-no(I,LAT,K))
...
contour,upp,alat,zpp,levels=level,/cell,c_color=col1,/noeras,color=mcolor,ytitle='Altitude (km)',xtitle='Latitude',xrange=[-90,90],title=sdate,charsize=2
...
;
ofile=dirw+'f.e11.FWTREFC1SD.f19.f19.ccmi30.001.cam.h5.'+sdate+'.nc'
print,'writing ',ofile
...
ncdf_varput, nocid, 'CO2' , co2grd  , COUNT=[nr,nc,nth]
ncdf_varput, nocid, 'NO' , nogrd  , COUNT=[nr,nc,nth]
ncdf_varput, nocid, 'NO2' , no2grd  , COUNT=[nr,nc,nth]
ncdf_close,nocid
jumpstep:
