...
spawn,'ls '+dirh+'*0101_QO3.sav',ifiles
...
; read WACCM data
;
      dum=findfile(dirh+'Year'+model_years(iyear)+'_'+sdate+'_QO3.sav')
...
;
      if icount eq 0L then begin
         QRS_SO3A00FPL_avg=fltarr(nr,nl,kday)
         QRS_SO3B00FPL_avg=fltarr(nr,nl,kday)
         QRS_LO3A00FPL_avg=fltarr(nr,nl,kday)
         QRS_LO3B00FPL_avg=fltarr(nr,nl,kday)
         QRS_LO300FPL_avg=fltarr(nr,nl,kday)
      endif
;
; loop over years and retain all data
;
      print,'restored '+dum(0)
      QRS_SO3A00FPL_avg(*,*,icount)=QRS_SO3A00FPL
      QRS_SO3B00FPL_avg(*,*,icount)=QRS_SO3B00FPL
      QRS_LO3A00FPL_avg(*,*,icount)=QRS_LO3A00FPL
      QRS_LO3B00FPL_avg(*,*,icount)=QRS_LO3B00FPL
      QRS_LO300FPL_avg(*,*,icount)=QRS_LO300FPL
;
jumpday:
      icount=icount+1L
goto,jump
...
;
saveit:
ofile=dirh+'Year'+model_years(iyear)+'_VLH_QO3.sav'
save,file=ofile,sdate_all,lat,lev,QRS_SO3A00FPL_avg,QRS_SO3B00FPL_avg,QRS_LO3A00FPL_avg,QRS_LO3B00FPL_avg,QRS_LO300FPL_avg
