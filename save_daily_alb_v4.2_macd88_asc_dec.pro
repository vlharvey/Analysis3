...
ALBLIM=2.       ;LOWER LIMIT FOR ALBEDO -- ANYTHING SMALLER THAN THIS IS ASSUMED NOT TO BE A CLOUD.^M
ERRLIM=1.0      ;MAXIMUM ALLOWED RATIO OF ALBEDO_ERR/ALBEDO^M
SZALIM_HI=94.   ;DATA WITH SZA > SZALIM_HI ARE SUSPECT^M
SZALIM_LO=42.   ;DATA WITH SZA < SZALIM_LO ARE SUSPECT^M
...
;
      spawn,'ls '+pth+'*'+syear+'-'+sday+'*v04.20_r04_cat.nc',fnamescat
      spawn,'ls '+pth+'*'+syear+'-'+sday+'*v04.20_r04_cld.nc',fnamescld
...
;
          good=WHERE(FINITE(ALB) EQ 1 and SZA LE SZALIM_HI AND SZA GE SZALIM_LO and ALB GE ALBLIM and RAD gt 20. and QUALITY_FLAGS eq 0.,ngood)	
          if ngood eq 0 then goto,skiporbit
...
;                    ALB Ge ALBLIM,ngood)	; and nlayers gt 6.,NGOOD)
          ...
          IF NGOOD GT 0 THEN BEGIN
             latitude_orig=latitude_orig(good)
             latitude=latitude(good)
             longitude=longitude(good)
;            ut_date=ut_date(good)		; v4.2 ut_date=one number YYYYDOY
          ...
          ENDIF
;
; retain daily variables
;
if kcount eq 0L then begin
   latitude_orig_all=latitude_orig
   latitude_all=latitude
   longitude_all=longitude
   ut_time_all=ut_time
   sza_all=sza
   CLOUD_INDEX_all=CLOUD_INDEX
   ALB_all=ALB
   ALB_ERR_all=ALB_ERR
   RAD_all=RAD
   RAD_ERR_all=RAD_ERR
...
if kcount gt 0L then begin
   latitude_orig_all=[latitude_orig_all,latitude_orig]
   latitude_all=[latitude_all,latitude]
   longitude_all=[longitude_all,longitude]
   ut_time_all=[ut_time_all,ut_time]
   sza_all=[sza_all,sza]
   CLOUD_INDEX_all=[CLOUD_INDEX_all,CLOUD_INDEX]
   ALB_all=[alb_all,ALB]
   ALB_ERR_all=[alb_err_all,ALB_ERR]
   RAD_all=[rad_all,rad]
   RAD_ERR_all=[rad_err_all,RAD_err]
   IWC_all=[IWC_all,IWC]
   IWC_ERR_all=[IWC_ERR_all,IWC_ERR]
   ICD_all=[icd_all,ICD]
endif
kcount=1

if min(alb) lt alblim then stop
skiporbit:
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='plot_daily_cipsl2_alb_vs_sza_colrad_v4.2_'+sdate+'.ps'
   !p.charsize=1.5
   !p.thick=1.5
   !p.charthick=5
   !p.charthick=5
...
plot,sza,alb,psym=3,color=0,xtitle='SZA',ytitle='Version 4.20 CIPS Albedo (10!u-6!n str!u-1!n)',title=sdate,xrange=[40.,90.],yrange=[0.,100.],/nodata
index=where(rad_all ge 1. and rad_all le 10.)
if index(0) ne -1L then oplot,sza_all(index),alb_all(index),psym=3,color=mcolor*.1
index=where(rad_all gt 10. and rad_all le 20.)
if index(0) ne -1L then oplot,sza_all(index),alb_all(index),psym=3,color=mcolor*.2
index=where(rad_all gt 20. and rad_all le 30.)
if index(0) ne -1L then oplot,sza_all(index),alb_all(index),psym=3,color=mcolor*.3
index=where(rad_all gt 30. and rad_all le 40.)
if index(0) ne -1L then oplot,sza_all(index),alb_all(index),psym=3,color=mcolor*.4
index=where(rad_all gt 40. and rad_all le 50.)
if index(0) ne -1L then oplot,sza_all(index),alb_all(index),psym=3,color=mcolor*.5
index=where(rad_all gt 50. and rad_all le 60.)
if index(0) ne -1L then oplot,sza_all(index),alb_all(index),psym=3,color=mcolor*.6
index=where(rad_all gt 60. and rad_all le 70.)
if index(0) ne -1L then oplot,sza_all(index),alb_all(index),psym=3,color=mcolor*.7
index=where(rad_all gt 70. and rad_all le 80.)
if index(0) ne -1L then oplot,sza_all(index),alb_all(index),psym=3,color=mcolor*.8
index=where(rad_all gt 90.)
if index(0) ne -1L then oplot,sza_all(index),alb_all(index),psym=3,color=mcolor*.9
slevel=['1','10','20','30','40','50','60','70','80','>90']
...
; Close PostScript file and return control to X-windows
 ...
 if setplot eq 'ps' then begin
    device, /close
    spawn,'convert -trim plot_daily_cipsl2_alb_vs_sza_colrad_v4.2_'+sdate+'.ps -rotate -90 '+$
                        'plot_daily_cipsl2_alb_vs_sza_colrad_v4.2_'+sdate+'.jpg'
    spawn,'rm -f plot_daily_cipsl2_alb_vs_sza_colrad_v4.2_'+sdate+'.ps'
 endif
;
; save daily file
;
save,file=ofile,latitude_orig_all,latitude_all,longitude_all,ut_time_all,sza_all,CLOUD_INDEX_all,ALB_all,ALB_ERR_all,$
     RAD_all,RAD_ERR_all,IWC_all,IWC_ERR_all,ICD_all
