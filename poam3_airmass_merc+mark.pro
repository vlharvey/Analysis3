...
; plot UKMO quantity in top panel
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='poam3_o3_'+stheta+$
...
;    o3min=2.
    ...
    xyouts,.35,.8,'POAM III Ozone '+stheta,/normal,charsize=2
       ...
       if index(0) ne -1 then begin
          ...
          for itraj=0,ntraj-1 do $
              oplot,[xn(itraj),xn(itraj)],[yn(itraj),yn(itraj)],psym=8,$
                    color=((o3n(itraj)-o3min)/(o3max-o3min))*mcolor,symsize=1.5
       endif
    endif
...
; color bar for Ozone data
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert poam3_o3_'+stheta+'_'+stime+'+mark_merc.ps -rotate -90 '+$
             ' poam3_o3_'+stheta+'_'+stime+'+mark_merc.jpg'
    endif
