...
CLOUD_FREQ_SIGMA_ALL=CLOUD_ALB_ALL
;
; ps file 
;
    if setplot eq 'ps' then begin
       ...
       device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
              /bold,/color,bits_per_pixel=8,/helvetica,filename='scatter_cipsl4_alb_vs_iwc.ps'
       !p.charsize=1.25
       !p.thick=2
       !p.charthick=5
       !p.charthick=5
...
;
    ...
    imin=0.
    imax=30.
    plot,cloud_freq_avg_lat2,cloud_freq_max_lat2,ytitle='IWC',xrange=[imin,imax],$
    ...
    if index(0) ne -1L then oplot,cloud_freq_avg_lat2(index),cloud_freq_max_lat2(index),color=icol,psym=8,symsize=0.7
...
;
    cloud_freq_avg_lat2=reform(cloud_freq_avg_all(*,ilat))
    cloud_freq_max_lat2=reform(cloud_freq_max_all(*,ilat))       ; fltarr(kday) at ilat2
    if ilat eq 0L then begin
    plot,cloud_freq_avg_lat2,cloud_freq_max_lat2,ytitle='IWC',xtitle='Cloud Albedo',xrange=[imin,imax],$
...
;
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim scatter_cipsl4_alb_vs_iwc.ps -rotate -90 scatter_cipsl4_alb_vs_iwc.jpg'
    endif
