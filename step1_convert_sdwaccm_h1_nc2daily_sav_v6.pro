...
ifiles=file_search(dir+'c_cesm2_fswd_2005_cntrl.cam.h1.20*-00000.nc',count=nfiles)
    ...
    for ivar=0,result0.nvars-1 do begin
        ...
        if result.name eq 'lat' or result.name eq 'lon' or result.name eq 'lev' or result.name eq 'P0' or result.name eq 'hyai' or $
           ...
           if result.name eq 'date' then begin
              ...
              result=file_search(dir+'c_cesm2_fswd_2005_cntrl.cam.h1.'+sdate+'.sav')
              if result(0) ne '' then ncdf_close,ncid
              if result(0) ne '' then goto,jumpstep		; if save file already exists then skip day
           endif
