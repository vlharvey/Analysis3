;
; plot mercator projection of multi-year seasonal average average albedo
;
loadct,39
device,decompose=0
mcolor=byte(!p.color)
...
;
; average seasonal averages
;
spawn,'ls '+pth+'*_v04.20_nh.sav',ifiles
nfiles=n_elements(ifiles)
for ifile=0,nfiles-1L do begin
    restore,ifiles(ifile)
    if ifile eq 0L then alb_avgall=alb_avg
    if ifile eq 0L then nalb_avgall=0.*alb_avg
    if ifile gt 0L then begin
       good=where(alb_avg gt 0.)
       alb_avgall(good)=alb_avgall(good)+alb_avg(good)
       nalb_avgall(good)=nalb_avgall(good)+1.
    endif
endfor
index=where(nalb_avgall gt 0.)
if index(0) ne -1L then alb_avgall(index)=alb_avgall(index)/nalb_avgall(index)
alb_avg=alb_avgall
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='merc_avgall_daisy_nh.ps'
   !p.charsize=1.5
   !p.thick=1.5
   !p.charthick=5
   !p.charthick=5
...
map_set,0,0,0,/contin,/grid,color=0,/noeras,title='CIPS L3 V4.20 NH 2007-2011',limit=[50, 0, 80, 360]
...
map_set,0,0,0,/contin,/grid,color=0,/noeras,limit=[50, 0, 80, 360]
imin=min(level)
imax=max(level)
ymnb=ymn -cbaryoff
ymxb=ymnb+cbarydel
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim merc_avgall_daisy_nh.ps -rotate -90 merc_avgall_daisy_nh.jpg'
;  spawn,'rm -f merc_avgall_daisy_nh.ps'
endif
end
