...
;MAKE SURE THAT THE COMMON FOLDER FROM CIPS TOOLS IS IN ONE FOLDER UP FROM THE CURRENT DIRECTORY.
;MAKE SURE THAT READ_CIPS_FILE.SAV IS IN THE CURRENT DIRECTORY.


;CHANGE ALBEDO LIMIT TO 2

season='SH0809'     ;NH07, NH08, SH0708, SH0809
season=strupcase(season)

HEM=STRMID(SEASON,0,2)
v='3.20'

;pth='e:\data\aim\cips\l4_nc\'
pthi='E:\cips\v'+v+'\'+season+'\gzfiles\'  ;DIRECTORY OF GZ FILES MUST BE IN THIS FOLDER (DIR.TXT)
ptho='c:\cips\analysis\v'+v+'\'+season+'\idlsave\'
...
;                END
;    'NH08':     BEGIN
;                   DAY1=121+366   ;MAY 1 (2008 = LEAP YEAR)
;                   DAY2=250+366   ;SEP 7
;                END
;    'SH0708':   BEGIN
;                   DAY1=305      ;NOV 1
...
;                END
;    'SH0809':   BEGIN
;                   DAY1=305+366   ;NOV 1 (2008 = LEAP YEAR)
;                   DAY2=431+366   ;MAR 7
;                END
;    'NH09':     BEGIN
;                   DAY1=121+366+365
...
ALBLIM=1   ;LOWER LIMIT FOR ALBEDO -- ANYTHING SMALLER THAN THIS IS ASSUMED NOT TO BE A CLOUD.
...
SZALIM_HI=91    ;DATA WITH SZA > SZALIM_HI ARE SUSPECT
SZALIM_LO=50    ;DATA WITH SZA < SZALIM_LO ARE SUSPECT

SCALIM = 6   ;MIN NUMBER OF SCATTERING ANGLES


;ALL DATA ARE ON DIFFERENT LAT/LON GRIDS, SO PUT THEM ONTO A COMMON (WACCM 4X5) GRID
NLAT=13 & NLON=72
LATGRID=INDGEN(NLAT)*(4.0)+42 ;WILL WORK FOR BOTH NH AND SH:
;FOR SH, TEMPORARILY CHANGE DATA LATITUDES TO BE POSITIVE SO THE SAME LOGIC CAN BE
;  USED AS FOR THE NH. THEN CHANGE LATGRID TO BE NEGATIVE AT THE END OF THE PROGRAM.
;  DO IT THIS WAY SO THAT WE CAN HANDLE LATITUDES ACROSS THE POLE EASILY, AND DON'T
;  HAVE TO CHANGE SIGN.
...
YYYY = STRMID(FNAMES,23,4)  ;USED TO ASSIGN THE DATE OF THE OUTPUT FILE
...
;FOR I = 0,0 DO BEGIN
   ...
   IWC_AVG = FLTARR(NLON,NLAT)-99
   IWC_ERR_AVG = FLTARR(NLON,NLAT)-99
   ...
   FOR J = 0,NO-1 DO BEGIN       ;LOOP OVER ORBITS ******
      FCOUNT=FCOUNT+1
      FNAME=FNAMES(FCOUNT)
      data=read_cips_file(pthi+fname,/full_path) ;Must have the path even if in the same directory!
      ;stop
      DAY = DATA.UT_DATE    ;YYYYDDD
      ;NOTE: WHEN THE ORBIT CROSSES THE UT DATE BOUNDARY, THIS DAY WILL *NOT* BE CONSISTENT
      ;   WITH THE DATE IN THE FILE NAME!! IT LOOKS LIKE THE FILE NAME HAS THE START DATE OF
      ;   THE ORBIT, WHEREAS THE DATE IN THE DATA HAS THE END DATE.
      ;CHANGE THIS LINE TO ONLY USE THE FILE NAME DATE:
      DAY=long(YYYY(FCOUNT)+DDD(FCOUNT))
      ;YYYYDDD -- CONVERTED TO YYYYMMDD LATER FOR USE IN IDLSAVE FILE NAME. MUST HAVE
      ;  IT AS LONG, NOT STRING, FOR YYYYDDD_TO_YYYYMMDD TO WORK.

      ALB = (*data[0].cld_albedo)
      ALB_ERR = (*DATA[0].CLD_ALBEDO_UNC)
      IF HEM EQ 'NH' THEN LATITUDE = (*DATA[0].LATITUDE)
      IF HEM EQ 'SH' THEN LATITUDE = (*DATA[0].LATITUDE) * (-1.0)
      ;stop
...
;PRINT,' '
      ...
      ;OMIT ANY ALBEDOS THAT ARE SMALLER THAN ALBLIM.
      ...
      ;   GT SZALIM ON THE DESCENDING NODE).
      BAD=WHERE(FINITE(ALB) EQ 0 OR FINITE(ALB_ERR) EQ 0 OR $
            SZA GT SZALIM_HI OR SZA LT SZALIM_LO,NBAD)
      ...
      FOR LO=0,NLON-1 DO BEGIN
         FOR LA=0,NLAT-1 DO BEGIN
            ...
            X=WHERE(LATITUDE GT LATGRID(LA)-LAINCR AND LATITUDE LE LATGRID(LA)+LAINCR $
                ...
                AND (CLOUD_INDEX EQ 1 OR CLOUD_INDEX EQ 0) $
                AND LAYER GE SCALIM,NX)
                ;STOP
            IF NX NE 0 THEN BEGIN
               NPTS(LO,LA,J) = NX  ;-99 IF NO VALID POINTS
               ;FIND CLOUDS
               Y=WHERE(CLOUD_INDEX(X) EQ 1 AND $
                       ALB(X) GE ALBLIM AND ALB_ERR(X) NE -99 AND ABS(ALB_ERR(X)/ALB(X)) LE ERRLIM,NY)
               IF NY GT 0 THEN BEGIN
                  ...
                  NCLDS(LO,LA,J) = NY
                  IWCS(LO,LA,J) = MEAN(IWC(X(Y)))
                  IWCS_ERR(LO,LA,J) = MEAN(IWC_ERR(X(Y)))
               ENDIF ELSE BEGIN
                  ALBS(LO,LA,J) = -99
                  ALBS_ERR(LO,LA,J) = -99
                  RADS(LO,LA,J) = -99
   ...
   FOR LO=0,NLON-1 DO BEGIN
      FOR LA=0,NLAT-1 DO BEGIN
         ...
         IF NX GT 0 THEN BEGIN
            ...
            IWC_AVG(LO,LA)=MEAN(IWCS(LO,LA,X))
            IWC_ERR_AVG(LO,LA) = MEAN(IWCS_ERR(LO,LA,X))
         ENDIF
         ;SO ALB_AVG AND ALB_ERR ONLY CORRESPOND TO CLOUD POINTS; OTHERWISE THEY ARE -99.
         ;DITTO FOR RADIUS & IWC.
...
;STOP
   ...
   IWC=IWC_AVG & IWC_ERR=IWC_ERR_AVG

   X=WHERE(NPOINTS LT NCLOUDS,NX)
   IF NX GT 0 THEN STOP

   IF HEM EQ 'SH' THEN LATGRID = LATGRID * (-1.0)   ;SAVE OUT NEGATIVE LATITUDES

   fout=ptho+strdt+'.dat'
   SAVE,LATGRID,LONGRID,ALBEDO,ALBEDO_ERR,NCLOUDS,NPOINTS,RADIUS,RADIUS_ERR,DT,$
   ERRLIM,SZALIM_HI,SZALIM_LO,SCALIM,ALBLIM,IWC,IWC_ERR,file=fout

   IF HEM EQ 'SH' THEN LATGRID = LATGRID * (-1.0)   ;PUT IT BACK TO POSITIVE FOR NEXT DAY

   ;PRINT, ' '
   ;PRINT,LATGRID
   ;PRINT,STRDT,' ',MIN(NCLOUDS),' ',MAX(NCLOUDS)  ;MAKE SURE IT'S NOT -99
   ;PRINT,' '
