...
;stop
			...
			;-------------------------------------READ IN AIRS DATA--***on the next day for squall case***-----------------------------------------
 		ncfile=path+'pert_airs_l1_2016_'+CIPS_doy+'.nc'
;		ncfile=path+'pert_airs_l1_2016_163.nc'
			;Open the *.nc file
...
;data = where(abs(airs_frac_day_all - ut_airs) le .25 and abs(lat-cips_lat_mean) lt 10. and (abs(lon - cips_lon_mean) lt 30.),nx)
data = where(abs(airs_frac_day_all - scenetime/24.) le .25 and abs(lat-cips_lat_mean) lt 10. and (abs(lon - cips_lon_mean) lt 30.),nx)	; more correct to use scenetime?
...
;set_plot,'x'
;window
;map_set,0,0,0,/contin,limit=[-55,65,-45,80],/stereo 
...
;stop
			...
			if cipsplot eq 1 and airsplot eq 1 then begin
				...
				plot_title = 'AIRS 4mu BT Pert, CIPS Albedo Anomaly!C'$
			...
			xyouts, .5,.76,plot_title,align=.5,charsize = 1.6
			loadct,0
...
;		if orbitlist[iorbit] eq '49885' then level2=[-0.4,-0.32,-0.24,-0.16,-0.12,-0.08,-0.04,0.0,0.04,0.08,0.12,0.16,0.24]	; Jeff's levels?
 			if orbitlist[iorbit] eq '49885' then level2=(2+level2)/4.
...
loadct,0
				...
				!p.title = ' '
				if airsplot eq 0 then position = [.3,0.25, .7,.32]
				if airsplot eq 1 then position = [.52,0.15, .9,.22]
			...
			if AIRSplot eq 1 then begin
				...
				!p.title = ' '
				if cipsplot eq 0 then position = [.3,0.15, .7,.22]
				if cipsplot eq 1 then position = [.1,0.15, .48,.22]
