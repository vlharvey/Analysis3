...
if iorbit eq 0L then begin
   if setplot eq 'ps' then begin
      ...
      device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
             /bold,/color,bits_per_pixel=8,/helvetica,filename='plot_daily_cipsl2_alb_vs_sza_coliwc_v4.1_'+sdate+'.ps'
      !p.charsize=1.5
      !p.thick=1.5
      !p.charthick=5
      !p.charthick=5
...
imax=60.
    ...
    plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],/noeras,color=0,xticks=nlvls-1,xtickname=slevel,$
          xtitle='Ice Water Content (g/m!u2!n)'
    ybox=[0,10,10,0,0]
    x2=imin
...
; Close PostScript file and return control to X-windows
     ...
     if setplot eq 'ps' then begin
        device, /close
        spawn,'convert -trim plot_daily_cipsl2_alb_vs_sza_coliwc_v4.1_'+sdate+'.ps -rotate -90 '+$
                            'plot_daily_cipsl2_alb_vs_sza_coliwc_v4.1_'+sdate+'.jpg'
        spawn,'rm -f plot_daily_cipsl2_alb_vs_sza_coliwc_v4.1_'+sdate+'.ps'
     endif
