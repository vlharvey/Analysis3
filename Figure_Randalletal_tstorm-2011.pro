...
for cipsplot = 1,1 do begin
	for airsplot = 0,0 do begin
		if cipsplot eq 0 and airsplot eq 0 then continue
		;-------------------------------------READ IN CIPS DATA-------------------------------------------
		for iorbit = 0, n_elements(orbitlist) - 1L do begin

			file=FILE_SEARCH(dir+'cips_raa_2a_orbit_'+orbitlist[iorbit]+'*_cat.nc',COUNT=nfiles)
...
;
                        ...
                        NCDF_CLOSE, ncid
                        file=FILE_SEARCH(dir+'cips_raa_2a_orbit_'+orbitlist[iorbit]+'*_alb.nc',COUNT=nfiles)
                        ncid=ncdf_open(file)
                        result=ncdf_inquire(ncid)   ;Inquire about the data
                        nvars=result.nvars        ;# variables in the file
                        for ivar=0,nvars-1 do begin
                        ...
                        CIPS_frac_day = 0.*jd_time
                        index=where(finite(jd_time))
                        CIPS_frac_day(index)=jd_time(index)-long(jd_time(index))+0.5	; add 0.5 since JD_TIME is since noon on 1 Jan 4713 BC
                        index=where(finite(CIPS_frac_day) and CIPS_frac_day gt 1.0)
                        if index(0) ne -1L then CIPS_frac_day(index)=CIPS_frac_day(index)-1.0
                        CIPS_time = 24.*CIPS_frac_day

scenetime=cips_time(iscene[iorbit])				; scene time in frac hours
...
;stop
			...
			sza_cips = ZENITH_ANGLE(*,*,iscene[iorbit])*180./!pi
...
;if bad(0) ne -1L then alb_pert_cips2(bad)=0./0.
			...
			xyouts, .5,.9,plot_title,align=.5,charsize = 1.6
			loadct,0
	
;limit = [cips_lat_mean-20,cips_lon_mean-30,cips_lat_mean+20,cips_lon_mean+30]; Set plot limits based on the mean CIPS lat/lon
limit = [cips_lat_mean-10,cips_lon_mean-10,cips_lat_mean+10,cips_lon_mean+10]; Set plot limits based on the mean CIPS lat/lon
;limit = [cips_lat_mean-4,cips_lon_mean-8,cips_lat_mean+5,cips_lon_mean+8]; Set plot limits based on the mean CIPS lat/lon
	
			Map_Set, cips_lat_mean, cips_lon_mean, /Satellite,  /NoErase, position = [.2,.2,.8,.8],$
			/ISOTROPIC, color=0, limit = limit, /horizon,$
			/CONTINENTS,/usa
                        MAP_CONTINENTS, fill_continents=1,color = 150
                        Map_Set, cips_lat_mean, cips_lon_mean, /Satellite,  /NoErase, position = [.2,.2,.8,.8],$^M
                        /ISOTROPIC, color=0, limit = limit, /horizon,$^M
                        /CONTINENTS,/usa

	
			;Set AIRS levels
			level1 = (findgen(11)*.2-1.)*3.
			if orbitlist[iorbit] eq '49885' then level1 = level1/30.
			if orbitlist[iorbit] eq '49883' then level1 = level1/3.
			if orbitlist[iorbit] eq '49877' then level1 = level1/8.
			...
			IF airsplot eq 1 then begin
				...
				for i = 0, n_elements(AIRS_plot_data) - 1L do begin
					if lats[i] gt limit[0] and lats[i] lt limit[2] and ((lons[i] gt limit[1]-3. and lons[i] lt (limit[3] +10)) $
...
;               col2 = [250,235,220,190,170,125,125,100,80,60,30,15,10,0]	; Jeff's colors?
                col2 = [250,235,220,190,125,125,80,60,30,15,10,0]	; for contour
;               col2 = [250,235,220,190,170,125,80,60,30,15,10,0]	; for oplot

 			if orbitlist[iorbit] eq '22442' then level2=(level2-1)/4.
...
;
; repeat low points for visibility
;
index=where(ac gt level2(2) and ac le level2(3))
if index(0) ne -1L then oplot,xc(index),yc(index),color=col2(3),psym=8,symsize=0.25

index=where(ac gt level2(1) and ac le level2(2))
if index(0) ne -1L then oplot,xc(index),yc(index),color=col2(2),psym=8,symsize=0.25
index=where(ac gt level2(0) and ac le level2(1))
if index(0) ne -1L then oplot,xc(index),yc(index),color=col2(1),psym=8,symsize=0.25
index=where(ac le level2(0))
if index(0) ne -1L then oplot,xc(index),yc(index),color=col2(0),psym=8,symsize=0.25
...
;        		MAP_GRID, /LABEL, LATLAB=floor(cips_lon_mean/15.)*15.-20., LONLAB=floor(cips_lat_mean/10.)*10+17, LATDEL=5, LONDEL=10
 		        MAP_GRID, /LABEL, LATLAB=floor(cips_lon_mean)+8., LONLAB=floor(cips_lat_mean)-3, LATDEL=5, LONDEL=5, charsize=2.5
;
...
; vortex edge
;
;for k=17,17 do begin
;    print,zprof(k) 
;      mark1=transpose(mark2(*,*,k))
;      mark=0.*fltarr(nc+1,nr)
;      mark(0:nc-1,0:nr-1)=mark1(0:nc-1,0:nr-1)
;      mark(nc,*)=mark(0,*)
;      contour,smooth(mark,3,/edge_truncate),alon2,alat,/overplot,levels=[0.5],thick=15,color=150
;endfor

lonarr = 67.5+findgen(33)*.2
latarr = -49.+0.*findgen(33)
oplot, lonarr, latarr, color = 0, thick = 14
loadct,70
