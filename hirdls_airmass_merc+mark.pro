...
dirh='/aura2/harvey/Airmass/HIRDLS/'
hfile=[$
'ukmo_hirdls_oct_2000_forward.airm']
kfile=n_elements(hfile)
for kk=0,kfile-1 do begin
close,11
openr,11,dirh+hfile(kk),/f77
read_hirdls_header,11,charexp,ukmo,nmc,ecmwf,restart,rfile,istime,$
...
;
    ...
    if ntraj gt 0 then $
       read_hirdls_airmass,11,ntraj,xn,yn,thn,agen,x0n,y0n,th0n,t0n,z0n,p0n,$
...
; plot UKMO quantity in top panel
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='hirdls_o3_'+stheta+$
...
;    o3min=0.
    ...
    xyouts,.35,.8,'HIRDLS Ozone '+stheta,/normal,charsize=2
...
; color bar for Ozone data
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert hirdls_o3_'+stheta+'_'+stime+'+mark_merc.ps -rotate -90 '+$
             ' hirdls_o3_'+stheta+'_'+stime+'+mark_merc.jpg'
    endif
