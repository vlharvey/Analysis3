;
; NH pre-PMC season May 1 to May 15 with the solstice on day 172
...
;
plotit:
;restore,'sza_nh_pre_'+syear+'_QF01_ALB1.sav'	;,asc_freq,des_freq,dfs,szabin,syear,albmax_asc,albmin_asc,albmax_des,albmin_des
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='sza_cloud_freq_v4.2_'+syear+'_nh_pre.ps'
   !p.charsize=1.5
   !p.thick=1.5
   !p.charthick=5
   !p.charthick=5
...
erase
xyouts,.1,.95,'CIPS Version 4.20 Cloud Frequency 1-15 May',/normal,color=0,charsize=1.75
...
save,file='sza_nh_pre_'+syear+'_QF01_ALB1.sav',asc_freq,des_freq,dfs,szabin,syear,albmax_asc,albmin_asc,albmax_des,albmin_des
;
; Close PostScript file and return control to X-windows
 if setplot ne 'ps' then stop
 if setplot eq 'ps' then begin
    device, /close
    spawn,'convert -trim sza_cloud_freq_v4.2_'+syear+'_nh_pre.ps -rotate -90 '+$
                        'sza_cloud_freq_v4.2_'+syear+'_nh_pre.jpg'
    spawn,'rm -f sza_cloud_freq_v4.2_'+syear+'_nh_pre.ps'
 endif
end
