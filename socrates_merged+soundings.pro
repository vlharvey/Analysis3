...
;                                      
;  Input data:	SOCRATES NOGAPS SOSST data
;  Input files: /Volumes/earth/aura6/data/NOGAPS_Alpha/Datfiles_SOSST/dmps_socrates.nogaps.2007 (has p, t, z, h2o and o3)
;  Output data:	SOCRATES NOGAPS merged and sounding data
;
;  Programed by: V. Lynn Harvey  Aug 2011  LASP
;
; IDL> restore,'dmps_socrates.nogaps.2007
; DATE            LONG      = Array[100514]
; DYNTROP         FLOAT     = Array[100514]
; H2O_PROF        FLOAT     = Array[100514, 35]		; already in ppmv
; ID              LONG      = Array[100514]
; LATITUDE        DOUBLE    = Array[100514]
; LONGITUDE       DOUBLE    = Array[100514]
; O3_PROF         FLOAT     = Array[100514, 35]		; already in ppmv
; PTHERMTROP      FLOAT     = Array[100514]
; PV_PROF         FLOAT     = Array[100514, 35]
; P_PROF          FLOAT     = Array[100514, 35]
; TH              FLOAT     = Array[35]
; THTHERMTROP     FLOAT     = Array[100514]
; TIME            DOUBLE    = Array[100514]
; TP_PROF         FLOAT     = Array[100514, 35]
; ZTHERMTROP      FLOAT     = Array[100514]
; Z_PROF          FLOAT     = Array[100514, 35]
;
...
odir='/Volumes/earth/aura6/data/NOGAPS_Alpha/Merged_data/'
sdir='/Volumes/earth/aura6/data/NOGAPS_Alpha/Sound_data/'
ddir='/Volumes/earth/aura6/data/NOGAPS_Alpha/Datfiles_SOSST/'
syear=['2008']
...
; restore data
;
    restore,ddir+'dmps_socrates.nogaps+noise.'+syear(iyear)
...
;
        ...
        for iday=1,mday(fix(smon(imonth))-1) do begin
            ...
            yymmdd=long(syear(iyear)+smon(imonth)+sday)
            norbit=0L & morbit=0L
...
; write forward merged data
;
             ofile='socrates_'+month(fix(smon(imonth))-1)+sday+'_'+syear(iyear)+'.merged'
             close,20
             openw,20,odir+ofile
             printf,20,yymmdd
             printf,20,morbit
...
;
             ...
             ofile='socrates_'+month(fix(smon(imonth))-1)+sday+'_'+syear(iyear)+'.merged.back'
             close,20
             openw,20,odir+ofile
             printf,20,yymmdd
             printf,20,morbit
...
; WRITE SOUNDING OUTPUT
             ...
             o3file='socrates_'+month(fix(smon(imonth))-1)+sday+'_'+syear(iyear)+'_o3.sound'
             h2ofile='socrates_'+month(fix(smon(imonth))-1)+sday+'_'+syear(iyear)+'_h2o.sound'
             close,21,25
...
; if no data
             ...
             if today(0) eq -1L or norbit lt 2L then begin
                close,19,20,21,25
                ofile1='socrates_'+month(fix(smon(imonth))-1)+sday+'_'+syear(iyear)+'.merged'
                ofile2='socrates_'+month(fix(smon(imonth))-1)+sday+'_'+syear(iyear)+'.merged.back'
...
print,syear(iyear)+smon(imonth)+sday+' ',norbit,morbit,min(o3_sound(index)),max(o3_sound)
