...
c        set dth/dt=0
c
c  Version NOGAPS - read NOGAPS-Alpha data generated by files from Karl Hoppel
c  /aura7/harvey/NOGAPS_Alpha/Datfiles/NOGAPSA_2008031012_MetO_sabmls_aim9c.nc3
c                                              VLH 8/2011
c***************************************************************************c
      program NOGAPS_traject_3d_v20
c
      include "netcdf.inc"
c
...
c
      ...
      REAL XO3OLD(NRmax,NCmax,nthwmx)
      REAL XH2OOLD(NRmax,NCmax,nthwmx)
C
      ...
      REAL XO3NEW(NRmax,NCmax,nthwmx)
      REAL XH2ONEW(NRmax,NCmax,nthwmx)
c
...
c
      print *, 'Opening NOGAPS_traject_3d_v20.fil'
      OPEN(5,FILE='NOGAPS_traject_3d_v20.fil',status='old')
...
c
      if(restart) then
        OPEN(13,FILE=dir(1:len)//rFILE,status='old',
     &       FORM='UNFORMATTED')
        call read_restart_data(13)
        close(13)
      endif
c
      if ( .not. restart) then
...
c
      ...
      DO 12 I=1,NC
        DO 12 J=1,NR
           ...
           xo3grd(j,i,l)=xo3sim(j,i,l)
           xh2ogrd(j,i,l)=xh2osim(j,i,l)
12    CONTINUE
      wmax=sqrt(wmax)
        ...
        DO 10 J = 1,NR
          DO 10 I = 1,NC
            IF(ITFLAG.NE.0)THEN
              ...
              XO3OLD(J,I,L)=XO3NEW(J,I,L)
              XH2OOLD(J,I,L)=XH2ONEW(J,I,L)
            ENDIF
            ...
            XO3NEW(J,I,L)=xo3sim(J,I,L)
            XH2ONEW(J,I,L)=xh2osim(J,I,L)
10      CONTINUE
C
...
c ******************************************************
              XO3GRD(J,I,L) = XO3NEW(J,I,L)+
     &          TSCALE*(XO3OLD(J,I,L)-XO3NEW(J,I,L))
              XH2OGRD(J,I,L) = XH2ONEW(J,I,L)+
     &          TSCALE*(XH2OOLD(J,I,L)-XH2ONEW(J,I,L))
20        CONTINUE
          wmax=sqrt(wmax)
...
c
c     print *,'wtime ',wtime,gmt,dtout,mod(gmt,dtout)
          if ( (abs(wtime).lt.0.1) .or. 
     +         (abs (abs(wtime)-abs(dtout)) .lt. 0.1) .or.
     +         (mod(gmt,dtout) .lt. 0.1) ) then		! Bug fix VLH 8/16/11
            if (ntraj .lt. 1) then
...
c Adding O3 variable                          VLH 8/11/2011
      write(iout)'o3_traj'
      WRITE(iout)(xo3n(j),j=1,ntraj)
c Adding H2O variable                         VLH 8/11/2011
      write(iout)'h2o_traj'
      WRITE(iout)(xh2on(j),j=1,ntraj)
...
c Add O3 variable                                    VLH 8/11/2011
      read(iunit) blank
      write(6,*)blank
      READ(iunit) (xo3n(j),j=1,ntraj)
c Add H2O variable                                   VLH 8/11/2011
      read(iunit) blank
      write(6,*)blank
      READ(iunit) (xh2on(j),j=1,ntraj)
...
c NOGAPS nc format
c
c 1   float longitudes(number_of_longitudes) ;
c 2   float latitudes(number_of_latitudes) ;
c 3   float th_levels(number_of_levels) ;
c 4   float ipv(number_of_levels, number_of_longitudes, number_of_latitudes) ;
c 5   float press(number_of_levels, number_of_longitudes, number_of_latitudes) ;
c 6   float msf(number_of_levels, number_of_longitudes, number_of_latitudes) ;
c 7   float u_wind(number_of_levels, number_of_longitudes, number_of_latitudes) ;
c 8   float v_wind(number_of_levels, number_of_longitudes, number_of_latitudes) ;
c 9   float th_dot(number_of_levels, number_of_longitudes, number_of_latitudes) ;
c 10  float qdf(number_of_levels, number_of_longitudes, number_of_latitudes) ;
c 11  float mark(number_of_levels, number_of_longitudes, number_of_latitudes) ;
c 12  float vel_pot(number_of_levels, number_of_longitudes, number_of_latitudes) ;
c 13  float strm(number_of_levels, number_of_longitudes, number_of_latitudes) ;
c 14  float ozone(number_of_levels, number_of_longitudes, number_of_latitudes) ;
c 15  float water(number_of_levels, number_of_longitudes, number_of_latitudes) ;
c
      ...
      call ncvgt(ncid, 14, istart, icount, arr12, iret)
      call ncvgt(ncid, 15, istart, icount, arr13, iret)
      ...
      do i=1,nc
        do j=1,nr
          do l=1,nth
             ...
             xmrksim(j,i,l)=arr11(j,i,l)
             xo3sim(j,i,l)=arr12(j,i,l)
             xh2osim(j,i,l)=arr13(j,i,l)
          enddo
        enddo
      enddo
      return
...
c level l
     ...
     +              +xscale*(qdfgrd(j,ip1,l)-qdfgrd(j,i,l))
              xo3j1=xo3grd(j,i,l)+xscale*(xo3grd(j,ip1,l)-xo3grd(j,i,l))
          xh2oj1=xh2ogrd(j,i,l)+xscale*(xh2ogrd(j,ip1,l)-xh2ogrd(j,i,l))
     ...
     +              +xscale*(qdfgrd(jp1,ip1,l)-qdfgrd(jp1,i,l))
              xo3jp1=xo3grd(jp1,i,l)
     +              +xscale*(xo3grd(jp1,ip1,l)-xo3grd(jp1,i,l))
              xh2ojp1=xh2ogrd(jp1,i,l)
     +              +xscale*(xh2ogrd(jp1,ip1,l)-xh2ogrd(jp1,i,l))
...
c level lp1
     ...
     +              +xscale*(qdfgrd(j,ip1,lp1)-qdfgrd(j,i,lp1))
               xo3j2=xo3grd(j,i,lp1)
     +              +xscale*(xo3grd(j,ip1,lp1)-xo3grd(j,i,lp1))
               xh2oj2=xh2ogrd(j,i,lp1)
     +              +xscale*(xh2ogrd(j,ip1,lp1)-xh2ogrd(j,i,lp1))
     ...
     +              +xscale*(qdfgrd(jp1,ip1,lp1)-qdfgrd(jp1,i,lp1))
              xo3jp2=xo3grd(jp1,i,lp1)
     +              +xscale*(xo3grd(jp1,ip1,lp1)-xo3grd(jp1,i,lp1))
              xh2ojp2=xh2ogrd(jp1,i,lp1)
     +              +xscale*(xh2ogrd(jp1,ip1,lp1)-xh2ogrd(jp1,i,lp1))
...
c level l
               ...
               xmrk1=xmrkj1+yscale*(xmrkjp1-xmrkj1)
                ...
                qdf1=qdfj1+yscale*(qdfjp1-qdfj1)
                  xo31=xo3j1+yscale*(xo3jp1-xo3j1)
                  xh2o1=xh2oj1+yscale*(xh2ojp1-xh2oj1)
c level lp1
               ...
               xmrk2=xmrkj2+yscale*(xmrkjp2-xmrkj2)
                ...
                xo32=xo3j2+yscale*(xo3jp2-xo3j2)
                xh2o2=xh2oj2+yscale*(xh2ojp2-xh2oj2)
c
...
c                pn(n)=p1+zscale*(p2-p1)
              ...
              xmrkn(n)=xmrk1+zscale*(xmrk2-xmrk1)
               ...
               qdfn(n)=qdf1+zscale*(qdf2-qdf1)
                 xo3n(n)=xo31+zscale*(xo32-xo31)
                 xh2on(n)=xh2o1+zscale*(xh2o2-xh2o1)
c
                goto 45 
              endif
   20       continue
...
c
      include 'comm_ukmo_3d_v20.'
      n=ntraj
      print *,' ntraj =',ntraj,' entering spacefill'
c     if (n .eq. 0) n = 1
      imax=360./dxs
      jmax=180./dys+1
...
c            n=n+1
      ...
      enddo
      ntraj=n
      print *,' ntraj =',ntraj,' exiting spacefill'
      return
      END
...
c
      n=ntraj
      print *,' ntraj =',ntraj,' entering er2_range_fill'
      if (n .eq. 0) n = 1
        ...
        if((xp.ge.xlon).and.(xp.le.xlonp1))then
          ...
          do 20 j=1,nr-1
            ...
            if((yp.ge.xlat).and.(yp.le.xlatp1))then
              yscale=(yp-xlat)/(xlatp1-xlat)
c
              uj1=ugrd(j,i,ll)+xscale*(ugrd(j,ip1,ll)-ugrd(j,i,ll))
              vj1=vgrd(j,i,ll)+xscale*(vgrd(j,ip1,ll)-vgrd(j,i,ll))
              qj1=qgrd(j,i,ll)+xscale*(qgrd(j,ip1,ll)-qgrd(j,i,ll))
c
          ujp1=ugrd(jp1,i,ll)+xscale*(ugrd(jp1,ip1,ll)-ugrd(jp1,i,ll))
          vjp1=vgrd(jp1,i,ll)+xscale*(vgrd(jp1,ip1,ll)-vgrd(jp1,i,ll))
          qjp1=qgrd(jp1,i,ll)+xscale*(qgrd(jp1,ip1,ll)-qgrd(jp1,i,ll))
c
...
c what is 0 < theta < 2*pi for igwave routine? DF 10/16/97
                  call igwave(uigw,vigw,u,v,i,j,ll,xp,yp,zp,wmag,theta) 
                  u=u+uigw
                  v=v+vigw
                endif
              endif
...
75           continue
      ...
      return
      END
      subroutine igwave(uigw,vigw,u,v,i,j,ll,xp,yp,zp,wmag,theta) 
c
c*****************************************************************c
c                                                                 c
...
c
      ...
      if(j.eq.nr)jp1=j
      if(j.eq.1)jm1=1
      dudy=ugrd(jm1,i,ll)-ugrd(jp1,i,ll)
      dvdx=vgrd(j,ip1,ll)-vgrd(j,im1,ll)
c      goto 10
