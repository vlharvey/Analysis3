...
;   varid = ncdf_varid(ncid,'Z3')	; no Z3! calculate approximate Z based on log lev and scale height
;   ncdf_varget,ncid,varid,gpdata
    gpdata=0.*tdata
    for i=0L,n_elements(instr_num)-1L do gpdata(*,i)=7000.*alog(1000./lev)
;
...
hdata=0.*clodata
    varid = ncdf_varid(ncid,'HNO3')		; changed from HNO3_GAS
