...
for icase=0L,n_elements(orbitlist)-1L do begin

spawn,'ls '+pth+syear+'_'+orbitlist(icase)+'_*.sav',fnames
...
alb_squall=reform(alb_scene(*,*,7))
sza_squall=reform(sza_scene(*,*,7))
;
; remove Nans
;
...
good=where(finite(ALB_SQUALL) and finite(SZA_SQUALL))
          alb_squall=alb_squall(good)
          sza_squall=sza_squall(good)
       ...
       if setplot eq 'ps' then begin
          ...
          device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
                 /bold,/color,bits_per_pixel=8,/helvetica,filename='mean_albpert_vs_sza_'+sorbit(iorbit)+'.ps'
          !p.charsize=1.25
          !p.thick=2
          !p.charthick=5
          !y.thick=2
...
ylim=2.e2
erase
plot,sza_scene,alb_scene,xrange=[50,105],yrange=[-1*ylim,ylim],xtitle='SZA',ytitle='Albedo perturbation (%)',color=0,psym=1,charsize=2,charthick=2,title=sorbit(iorbit)
oplot,sza_squall,alb_squall,psym=1,color=250
stop

     if setplot ne 'ps' then stop
     if setplot eq 'ps' then begin
        device, /close
        spawn,'convert -trim mean_albpert_vs_sza_'+sorbit(iorbit)+'.ps -rotate -90 mean_albpert_vs_sza_'+sorbit(iorbit)+'.jpg'
        spawn,'rm -f mean_albpert_vs_sza_'+sorbit(iorbit)+'.ps
     endif
...
; save
;
save,file='mean_albedo_vs_sza.sav',nsza,szabin,norbit,sorbit_all,cips_mean,cips_sigma,cips_counts
quick:
restore,'mean_albedo_vs_sza.sav'	;,nsza,szabin,norbit,sorbit_all,cips_mean,cips_sigma,cips_counts
;
; postscript file
;
       if setplot eq 'ps' then begin
          ...
          !p.font=0
          device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
                 /bold,/color,bits_per_pixel=8,/helvetica,filename='mean_albpert_vs_sza.ps'
          !p.charsize=1.25
          !p.thick=2
          !p.charthick=5
          !y.thick=2
...
!type=2^2+2^3
ylim=5.e0
plot,szabin,reform(cips_mean(*,0)),yrange=[-1*ylim,ylim],xtitle='SZA',ytitle='Albedo perturbation (%)',color=0,thick=5,/nodata,charsize=2,charthick=2,title='March 22 - August 4 2016'
...
; Close PostScript file and return control to X-windows
     ...
     if setplot eq 'ps' then begin
        device, /close
        spawn,'convert -trim mean_albpert_vs_sza.ps -rotate -90 mean_albpert_vs_sza.jpg'
;       spawn,'rm -f mean_albpert_vs_sza.ps
     endif
