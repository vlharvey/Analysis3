...
;
    ...
    col1=1.+indgen(nlvls)*mcolor/nlvls
    contour,reform(zmap(*,*,ilev,n)),xnd,ynd,levels=level,/cell_fill,c_color=col1,/noeras,/overplot

    zmn=min(level)
    zmx=max(level)
    for j=0L,jmax-1L,1 do begin
    for i=0L,imax-1L,1 do begin
    if xmrksfmap(i,j,ilev,0) gt 0. and ymap(i,j,ilev,0) gt 40. and n gt 0 then $
       oplot,[xmap(i,j,ilev,n),xmap(i,j,ilev,n)],[ymap(i,j,ilev,n),ymap(i,j,ilev,n)],psym=4,symsize=1.5,color=((zmap(i,j,ilev,n)-zmn)/(zmx-zmn))*mcolor
    if xmrksfmap(i,j,ilev,0) lt 0. and ymap(i,j,ilev,0) gt 40. and n gt 0 then $
       oplot,[xmap(i,j,ilev,n),xmap(i,j,ilev,n)],[ymap(i,j,ilev,n),ymap(i,j,ilev,n)],psym=8,symsize=1.5,color=((zmap(i,j,ilev,n)-zmn)/(zmx-zmn))*mcolor
    endfor
    endfor
    ...
    contour,sf1,x,alat,/follow,color=0,nlevels=10,thick=5,/overplot,c_labels=[0]
...
!type=2^2+2^3+2^6
plot,[kmin,kmax],[0,0],yrange=[0,10],xrange=[kmin,kmax],/noeras,color=0,xtitle='GPH (km)',charsize=2,charthick=2
ybox=[0,10,10,0,0]
x2=kmin
