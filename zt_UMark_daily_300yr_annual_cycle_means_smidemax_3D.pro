;
; plot average annual cycle of U and Mark in the polar regions
;
loadct,39
mcolor=byte(!p.color)
icmm1=mcolor-1B
...
;
    if setplot eq 'ps' then begin
       ...
       device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
              /bold,/color,bits_per_pixel=8,/helvetica,filename='zt_UMark_NH_SH_smidemax.ps'
       !p.charsize=1.25
       !p.thick=2
       !p.charthick=2
       !y.thick=2
...
;
    ...
    contour,unh,findgen(ndays),znh,/noera,/fill,color=0,c_color=col1,levels=tlevel,xrange=[0,ndays-1],yrange=[30,125],ytitle='Altitude (km)',charsize=1.5,charthick=2,title='NH',$
            xticks=nxticks-1,xtickname=xlabs,xtickv=xindex
    contour,unh,findgen(ndays),znh,/noera,/foll,color=0,thick=12,levels=[0],/overplot
    index=where(tlevel gt 0.)
    contour,unh,findgen(ndays),znh,/noera,/foll,color=0,thick=2,levels=tlevel(index),/overplot
    index=where(tlevel lt 0.)
    contour,unh,findgen(ndays),znh,/noera,/foll,color=mcolor,thick=2,levels=tlevel(index),/overplot,c_linestyle=5
    index=where(mnh gt 0.)
    if index(0) ne -1L then oplot,x2d(index),znh(index),psym=8,color=0,symsize=0.5
    index=where(mnh lt 0.)
    if index(0) ne -1L then oplot,x2d(index),znh(index),psym=8,color=mcolor,symsize=0.5
...
;   contour,mnh,findgen(ndays),znh,/noera,/foll,color=mcolor,thick=2,levels=-1+0.1*findgen(10),/overplot
    ...
    ymx=yorig(1)+ylen
    set_viewport,xmn,xmx,ymn,ymx
    contour,ush,findgen(ndays),zsh,/noera,/fill,color=0,c_color=col1,levels=tlevel,xrange=[0,ndays-1],yrange=[30,125],ytitle='Altitude (km)',charsize=1.5,charthick=2,title='SH',$
            xticks=nxticks-1,xtickname=xlabs,xtickv=xindex
    contour,ush,findgen(ndays),zsh,/noera,/foll,color=0,thick=12,levels=[0],/overplot
    index=where(tlevel gt 0.)
    contour,ush,findgen(ndays),zsh,/noera,/foll,color=0,thick=2,levels=tlevel(index),/overplot
    index=where(tlevel lt 0.)
    contour,ush,findgen(ndays),zsh,/noera,/foll,color=mcolor,thick=2,levels=tlevel(index),/overplot,c_linestyle=5
    index=where(msh gt 0.)
    if index(0) ne -1L then oplot,x2d(index),zsh(index),psym=8,color=0,symsize=0.5
    index=where(msh lt 0.)
    if index(0) ne -1L then oplot,x2d(index),zsh(index),psym=8,color=mcolor,symsize=0.5
...
xyouts,(x0+x1)/3.,y0-0.03,'60deg Zonal Wind (m/s)',color=0,charsize=1.5,charthick=2,/normal
...
;
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim zt_UMark_NH_SH_smidemax.ps -rotate -90 zt_UMark_NH_SH_smidemax.jpg
;      spawn,'rm -f zt_UMark_NH_SH_smidemax.ps'
    endif
