...
;
    if setplot eq 'ps' then begin
       ...
       device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
              /bold,/color,bits_per_pixel=8,/helvetica,filename='zt_TUMarkarea_NH_SH_smidemax.ps'
       !p.charsize=1.25
       !p.thick=2
       !p.charthick=2
       !y.thick=2
...
level=55+2.5*findgen(13)
nlvls=n_elements(level)
col1=reverse((findgen(nlvls)/float(nlvls))*mcolor)
contour,vareamean_ztnh,findgen(ndays),znh,/noera,/foll,color=0,c_color=col1,levels=level,thick=10,xrange=[0,ndays-1],yrange=[30,125],ytitle='Altitude (km)',charsize=1.5,charthick=2,title='NH Vortex',$
...
ymx=yorig(1)+ylen
set_viewport,xmn,xmx,ymn,ymx
contour,hareamean_ztnh,findgen(ndays),znh,/noera,/foll,color=0,c_color=col1,levels=level,thick=10,xrange=[0,ndays-1],yrange=[30,125],charsize=1.5,charthick=2,title='NH Highs',$
            xticks=nxticks-1,xtickname=xlabs,xtickv=xindex
xmn=xorig(2)
xmx=xorig(2)+xlen
ymn=yorig(2)
ymx=yorig(2)+ylen
set_viewport,xmn,xmx,ymn,ymx
contour,vareamean_ztsh,findgen(ndays),zsh,/noera,/foll,color=0,c_color=col1,levels=level,thick=10,xrange=[0,ndays-1],yrange=[30,125],ytitle='Altitude (km)',charsize=1.5,charthick=2,title='SH Vortex',$
            xticks=nxticks-1,xtickname=xlabs,xtickv=xindex
xmn=xorig(3)
xmx=xorig(3)+xlen
ymn=yorig(3)
ymx=yorig(3)+ylen
set_viewport,xmn,xmx,ymn,ymx
contour,hareamean_ztsh,findgen(ndays),zsh,/noera,/foll,color=0,c_color=col1,levels=level,thick=10,xrange=[0,ndays-1],yrange=[30,125],charsize=1.5,charthick=2,title='SH Highs',$
            xticks=nxticks-1,xtickname=xlabs,xtickv=xindex
;
...
xyouts,(x0+x1)/3.,y0-0.03,'Area Equivalent Latitude',color=0,charsize=1.5,charthick=2,/normal
...
;
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim zt_TUMarkarea_NH_SH_smidemax.ps -rotate -90 zt_TUMarkarea_NH_SH_smidemax.jpg
;      spawn,'rm -f zt_TUMarkarea_NH_SH_smidemax.ps'
    endif
