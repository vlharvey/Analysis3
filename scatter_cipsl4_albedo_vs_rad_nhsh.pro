...
;
    if setplot eq 'ps' then begin
       ...
       device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
              /bold,/color,bits_per_pixel=8,/helvetica,filename='scatter_cipsl4_alb_vs_rad.ps'
       !p.charsize=1.25
       !p.thick=2
       !p.charthick=5
       !p.charthick=5
...
;
    cloud_freq_avg_lat2=reform(cloud_freq_avg_all(*,ilat2))	; alb
    cloud_freq_max_lat2=reform(cloud_freq_max_all(*,ilat2))	; iwc
    cloud_freq_sigma_lat2=reform(cloud_freq_sigma_all(*,ilat2))	; rad
    if ilat eq 0L then begin
    imin=0.
    imax=30.
    plot,cloud_freq_avg_lat2,cloud_freq_sigma_lat2,ytitle='Radius',xrange=[imin,imax],$
         charsize=1.5,color=0,title='CIPS Level 4 V3.20',yrange=[40.,70.],/nodata
    endif
    syear=strmid(strcompress(sdate_all,/remove_all),0,4)
    index=where(cloud_freq_avg_lat2 ge 0.1)
    if index(0) ne -1L then oplot,cloud_freq_avg_lat2(index),cloud_freq_sigma_lat2(index),color=icol,psym=8,symsize=0.7
    if latbin(ilat) le -60. then xyouts,xmx+0.02,ymx-0.05-0.05*ilat,slat+'N',color=icol,/normal,charsize=1.5,charthick=5
skiplatnh:
endfor
xyouts,xmn+0.15,ymn+0.02,min(sdate_all)+'-'+max(sdate_all),/normal,charsize=1.5,color=0
...
;
    cloud_freq_avg_lat2=reform(cloud_freq_avg_all(*,ilat))	; alb
    cloud_freq_max_lat2=reform(cloud_freq_max_all(*,ilat))      ; iwc
    cloud_freq_sigma_lat2=reform(cloud_freq_sigma_all(*,ilat)) ; rad
    if ilat eq 0L then begin
    plot,cloud_freq_avg_lat2,cloud_freq_sigma_lat2,ytitle='Radius',xtitle='Cloud Albedo',xrange=[imin,imax],$
         charsize=1.5,color=0,yrange=[40.,70.],/nodata
    endif
    syear=strmid(strcompress(sdate_all,/remove_all),0,4)
    index=where(cloud_freq_avg_lat2 ge 0.1)
    if index(0) ne -1L then oplot,cloud_freq_avg_lat2(index),cloud_freq_sigma_lat2(index),color=icol,psym=8,symsize=0.7
...
;
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim scatter_cipsl4_alb_vs_rad.ps -rotate -90 scatter_cipsl4_alb_vs_rad.jpg'
    endif
