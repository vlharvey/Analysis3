...
CLOUD_FREQ_AVG_ALL=CLOUD_iwc_ALL
CLOUD_FREQ_MAX_ALL=CLOUD_iwc_ALL
CLOUD_FREQ_SIGMA_ALL=CLOUD_iwc_ALL
;
; ps file 
;
    if setplot eq 'ps' then begin
       ...
       device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
              /bold,/color,bits_per_pixel=8,/helvetica,filename='timeseries_cipsl4_cloudiwc_all_lat.ps'
       !p.charsize=1.25
       !p.thick=2
       !p.charthick=5
       !p.charthick=5
...
;   cloud_freq_avg_lat2=smooth(reform(cloud_freq_avg_all(*,ilat2)),3)       ; fltarr(kday) at ilat2
    ...
    plot,findgen(ndoy),cloud_freq_max_lat2,ytitle='Cloud IWC',xrange=[imin,imax],$
         xticks=nxticks,xtickname=xlab,xtickv=float(xlab),charsize=1.5,color=0,/nodata,$
         title='CIPS Level 4 V3.20',yrange=[0.,ymax]
    endif
...
;   cloud_freq_avg_lat=smooth(reform(cloud_freq_avg_all(*,ilat)),3)     ; fltarr(kday) at ilat
    ...
    xlab=['-40','-20','0','20','40','60','80']
    nxticks=n_elements(xtickname)
    plot,findgen(ndoy),cloud_freq_max_lat,ytitle='Cloud IWC',xrange=[imin,imax],$
         xticks=nxticks,xtickname=xlab,xtickv=float(xlab),charsize=1.5,color=0,/nodata,$
         yrange=[0.,ymax],xtitle='Days from Solstice'
    endif
    syear=strmid(strcompress(sdate_all,/remove_all),0,4)
;
    shsol=355.
...
;
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim timeseries_cipsl4_cloudiwc_all_lat.ps -rotate -90 timeseries_cipsl4_cloudiwc_all_lat.jpg'
    endif
