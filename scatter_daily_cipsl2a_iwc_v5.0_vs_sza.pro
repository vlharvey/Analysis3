;
; read CIPS level 2a version 5.00 data and plot IWC for all orbits each day
;
loadct,0
mcolor=!p.color
...
;
    if setplot eq 'ps' then begin
       ...
       device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
              /bold,/color,bits_per_pixel=8,/helvetica,filename='scatter_daily_cipsl2a_iwc_v5.0_vs_sza_'+sdate+'.ps'
       !p.charsize=1.25
       !p.thick=2
       !p.charthick=5
       !p.charthick=5
   ...
   !type=2^2+2^3
   plot,sza,iwc,xrange=[50,120],yrange=[-300,300],psym=3,color=0,xtitle='SZA',ytitle='IWC (ug/m!u2!n)',charsize=2,charthick=2
   xyouts,90,250,sdate,/data,charsize=3,charthick=2,color=0
   plots,90,0
   plots,90,100,/continue,color=0,thick=3

endif
oplot,sza,iwc,psym=8,color=col1(iorbit)
...
if index(0) ne -1L then oplot,sza_all(index),iwc_all(index),psym=8,color=250,symsize=0.5
loadct,0
...
; Close PostScript file and return control to X-windows
     ...
     if setplot eq 'ps' then begin
        device, /close
        spawn,'convert -trim scatter_daily_cipsl2a_iwc_v5.0_vs_sza_'+sdate+'.ps -rotate -90 scatter_daily_cipsl2a_iwc_v5.0_vs_sza_'+sdate+'.jpg'
        spawn,'rm -f scatter_daily_cipsl2a_iwc_v5.0_vs_sza_'+sdate+'.ps'
     endif
