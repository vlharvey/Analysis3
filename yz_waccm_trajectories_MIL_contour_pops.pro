...
nh=1
ifilet=['../WACCM_20060115_10day_fdf_MIL.traj']
ifilet=['../WACCM_20060115_37day_fdf.traj']
...
; comment this out and plot yrange=[-5,5]
    contour,tbar,alat,zbar,/fill,c_color=col1,levels=level,xrange=[0.,90.],yrange=[65.,80.],ytitle='Geopotential Height (km)',xtitle='Latitude',charsize=2,charthick=2
...
;oplot,[ymap(i,j,ilev,ii),ymap(i,j,ilev,ii)],[zmap(i,j,ilev,ii),zmap(i,j,ilev,ii)],psym=8,symsize=2,color=(float(ii)/float(nfiles))*mcolor
;oplot,[ymap(i,j,ilev,ii),ymap(i,j,ilev,ii+1)],[zmap(i,j,ilev,ii),zmap(i,j,ilev,ii+1)],psym=0,thick=2,color=(float(ii)/float(nfiles))*mcolor
;endfor
;endfor
;endfor
...
!type=2^2+2^3+2^6
plot,[kmin,kmax],[0,0],yrange=[0,10],xrange=[kmin,kmax],/noeras,color=0,xtitle='WACCM Trajectory Age (days)',charsize=2,charthick=2
ybox=[0,10,10,0,0]
x2=kmin
