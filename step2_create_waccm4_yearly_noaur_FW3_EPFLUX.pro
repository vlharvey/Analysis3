...
;
      if icount eq 0L then begin
         EPDNAFPL_avg=fltarr(nr,nl,kday)
         FYNAFPL_avg=fltarr(nr,nl,kday)
         FZNAFPL_avg=fltarr(nr,nl,kday)
         VRESNAFPL_avg=fltarr(nr,nl,kday)
         WRESNAFPL_avg=fltarr(nr,nl,kday)
      endif
;
; loop over years and retain all data
;
      print,'restored '+dum(0)
      EPDNAFPL_avg(*,*,icount)=EPDNAFPL
      FYNAFPL_avg(*,*,icount)=FYNAFPL
      FZNAFPL_avg(*,*,icount)=FZNAFPL
      VRESNAFPL_avg(*,*,icount)=VRESNAFPL
      WRESNAFPL_avg(*,*,icount)=WRESNAFPL
;
jumpday:
      icount=icount+1L
goto,jump
...
save,file=ofile,sdate_all,lat,lev,EPDNAFPL_avg,FYNAFPL_avg,FZNAFPL_avg,VRESNAFPL_avg,WRESNAFPL_avg
