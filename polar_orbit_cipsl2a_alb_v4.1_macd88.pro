...
;
          good=WHERE(FINITE(ALB) EQ 1,ngood)	; AND FINITE(ALB_ERR) EQ 1 AND $
;                    SZA LT SZALIM_HI AND SZA GT SZALIM_LO AND $ 
;                   (CLOUD_INDEX EQ 1 OR CLOUD_INDEX EQ 0) AND $
;                    ALB GE ALBLIM and nlayers gt 6.,NGOOD)
          ...
          IF NGOOD GT 0 THEN BEGIN
             ...
             PERCENT_CLOUDS=PERCENT_CLOUDS(good)
...
;
    if setplot eq 'ps' then begin
       ...
       device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
              /bold,/color,bits_per_pixel=8,/helvetica,filename='polar_orbit_cipsl2_alb_v4.1_'+sdate+'_'+sorbit+'.ps'
       !p.charsize=1.25
       !p.thick=2
       !p.charthick=5
       !p.charthick=5
...
;if iorbit eq 0L then begin
   ...
   !type=2^2+2^3
   map_set,90,0,-90,/stereo,/contin,/grid,title=sdate+' '+sorbit,color=0,/usa,limit=[30.,0.,90.,360.]
;endif
amin=0.
amax=60.
...
; Close PostScript file and return control to X-windows
     ...
     if setplot eq 'ps' then begin
        device, /close
        spawn,'convert -trim polar_orbit_cipsl2_alb_v4.1_'+sdate+'_'+sorbit+'.ps -rotate -90 '+$
                            'polar_orbit_cipsl2_alb_v4.1_'+sdate+'_'+sorbit+'.jpg'
        spawn,'/usr/bin/rm polar_orbit_cipsl2_alb_v4.1_'+sdate+'_'+sorbit+'.ps'
     endif
