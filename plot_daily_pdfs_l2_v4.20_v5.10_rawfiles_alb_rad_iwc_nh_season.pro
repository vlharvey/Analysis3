;
; plot daily pdfs - include full season
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='compare_cips_l4_v4.20_v5.10_rawfiles_nh_'+syear+'.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !p.charthick=5
...
; --- Test for end condition and close windows.
      ...
      if icount gt 0L then begin
         a20_all=a20_all+ALB_PDF42
         a51_all=a51_all+ALB_PDF51
         i20_all=i20_all+IWC_PDF42
         i51_all=i51_all+IWC_PDF51
         r20_all=r20_all+RAD_PDF42
         r51_all=r51_all+RAD_PDF51
      endif
      icount=icount+1
      skipcips:
goto,jump
...
; average
;
a20_all=a20_all/float(icount)
a51_all=a51_all/float(icount)
i20_all=i20_all/float(icount)
i51_all=i51_all/float(icount)
r20_all=r20_all/float(icount)
r51_all=r51_all/float(icount)

erase
xyouts,.4,.8,syear,/normal,color=0,charsize=3,charthick=2
!type=2^2+2^3
xmn=xorig(0)
xmx=xorig(0)+xlen
ymn=yorig(0)
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim compare_cips_l4_v4.20_v5.10_rawfiles_nh_'+syear+'.ps -rotate -90 compare_cips_l4_v4.20_v5.10_rawfiles_nh_'+syear+'.jpg'
;  spawn,'rm -f compare_cips_l4_v4.20_v5.10_rawfiles_nh_'+syear+'.ps'
endif
