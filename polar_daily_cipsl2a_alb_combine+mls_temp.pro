...
;
    if setplot eq 'ps' then begin
       ...
       device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
              /bold,/color,bits_per_pixel=8,/helvetica,filename='polar_daily_cipsl2a_alb_'+sdate+'_combine+mls_temp.ps'
       !p.charsize=1.25
       !p.thick=2
       !p.charthick=5
       !p.charthick=5
...
;
; wrap around point for MLS grid
;
      lonbin2=fltarr(nlon+1)
      lonbin2(0:nlon-1)=lonbin
      lonbin2(nlon)=lonbin(0)+360.
      mlst1=mls3d(*,*,thlev0m)
      mlst=-9999.+0.*fltarr(nlon+1,nlat)
...
;contour,mlst,lonbin2,latbin,/overplot,levels=level,c_color=col1,/cell_fill,/noeras
;contour,mlst,lonbin2,latbin,/overplot,levels=level,/follow,color=0,c_labels=0*level
;
; get CIPS nc filenames on this day
;
...
;
; comment out for quick check of MLS
;
...
;if index(0) ne -1L then oplot,longitude(index),latitude(index),psym=3,color=mcolor*.9
;
; superimpose MLS temperature
;
contour,mlst,lonbin2,latbin,/overplot,levels=130.+3.*findgen(21),/follow,color=0,c_labels=1+0*indgen(21),thick=3
contour,mlst,lonbin2,latbin,/overplot,levels=150.,/follow,color=0,c_labels=[1],thick=10
...
;        spawn,'convert -trim polar_daily_cipsl2a_alb_'+sdate+'_combine+mls_temp.ps -rotate -90 '+$
;                            'polar_daily_cipsl2a_alb_'+sdate+'_combine+mls_temp.jpg'
;print,'converted'
;       spawn,'/usr/bin/rm polar_daily_cipsl2a_alb_combine+mls_temp'+sdate+'.ps'
     endif
