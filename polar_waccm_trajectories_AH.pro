; 
; plot WACCM trajectories polar plot
; 
loadct,39
device,decompose=0
mcolor=255
...
;   xmrksfavemap(imax,*,*,*)=xmrksfavemap(0,*,*,*)
    ...
    if setplot eq 'ps' then begin
       ...
       !p.charthick=2
       device,font_size=9
       device,/color,/landscape,bits=8,filename='polar_trajectories_waccm_'+stime1+'.ps'
...
contour,tmp1,x,alat,levels=level,/follow,/overplot,/noeras,color=0

;oplot,xmap(*,*,ilev,0),zmap(*,*,ilev,0),psym=8,color=0
zmn=fix(min(zmap(*,jmax/2:jmax-1,ilev,*))-1.0)
zmx=round(max(zmap(*,jmax/2:jmax-1,ilev,*))+1.0)
zmn=-13.
zmx=13.
for j=0L,jmax-1L,1 do begin
for i=0L,imax-1L,1 do begin
;oplot,[xmap(i,j,ilev,n),xmap(i,j,ilev,n+1)],[ymap(i,j,ilev,n),ymap(i,j,ilev,n+1)],psym=0,color=((zmap(i,j,ilev,n)-zmap(i,j,ilev,0)+zmn)/(zmx-zmn))*mcolor,thick=5
if xmrksfmap(i,j,ilev,0) gt 0. and ymap(i,j,ilev,0) gt 40. and n gt 0 then begin
   for ii=n,n do begin
       oplot,[xmap(i,j,ilev,ii),xmap(i,j,ilev,ii)],[ymap(i,j,ilev,ii),ymap(i,j,ilev,ii)],psym=4,symsize=1.5,color=((zmap(i,j,ilev,n)-zmap(i,j,ilev,0)+zmn)/(zmx-zmn))*mcolor
   endfor
endif
if xmrksfmap(i,j,ilev,0) lt 0. and ymap(i,j,ilev,0) gt 40. and n gt 0 then begin
   for ii=n,n do begin
       oplot,[xmap(i,j,ilev,ii),xmap(i,j,ilev,ii)],[ymap(i,j,ilev,ii),ymap(i,j,ilev,ii)],psym=8,symsize=1.5,color=((zmap(i,j,ilev,n)-zmap(i,j,ilev,0)+zmn)/(zmx-zmn))*mcolor
   endfor
endif
...
!type=2^2+2^3+2^6
plot,[kmin,kmax],[0,0],yrange=[0,10],xrange=[kmin,kmax],/noeras,color=0,xtitle='Trajectory Delta Height (km)',charsize=2,charthick=2
ybox=[0,10,10,0,0]
x2=kmin
...
if setplot eq 'ps' then begin
   device,/close
   spawn,'convert polar_trajectories_waccm_'+stime1+'.ps -rotate -90 polar_trajectories_waccm_'+stime1+'.jpg'
;  spawn,'rm -f polar_trajectories_waccm_'+stime1+'.ps'
endif
endfor
end
