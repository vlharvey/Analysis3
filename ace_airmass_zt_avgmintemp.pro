...
if index(0) ne -1L then begin
   tmin(itime(0),klev(0))=tmin(itime(0),klev(0))+total(mintday(index))
   ntmin(itime(0),klev(0))=ntmin(itime(0),klev(0))+nt
endif
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='ace_airmass_zt_avgmintemp_'+stime0+'-'+stime1+'.ps'
   device,/color
...
contour,rtmin,findgen(nfiles),thlev,levels=level,/noeras,/cell_fill,c_color=col1,$
    ...
    title='ACE Avg Min Vortex Temperature in Previous 10 days',charsize=2,$
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim ace_airmass_zt_avgmintemp_'+stime0+'-'+stime1+'.ps -rotate -90 '+$
         ' ace_airmass_zt_avgmintemp_'+stime0+'-'+stime1+'.jpg'
endif
