...
;sdate='20130713'
;sdate='20130711'
;isuff=['1month','2weeks','10days','8days','6days','4days']
isuff=['2weeks']
...
if index(0) ne -1L then h2omap(index)=9999.
    ...
    if setplot eq 'ps' then begin
       ...
       device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
              /bold,/color,bits_per_pixel=8,/helvetica,filename='rdf_socrates_'+sdate+'_2pan+xys_'+isuff(nn)+'.ps'
       !p.charsize=1.25
       !p.thick=2
       !p.charthick=5
       !y.thick=2
...
;index=where(o3map eq 9999.)
;if index(0) ne -1L then o3map(index)=0./0.
;o3map=smooth(o3map,3,/Nan,/edge_truncate)
;
; smooth
;
...
;   xyouts,.425,.95,sdate,/normal,charsize=1.5,color=0,charthick=3
    ...
    pv(*,-1)=0./0.
    map_set,/MOLLWEIDE,0,140,0,/noeras,color=0,/contin,/grid,/isotropic,/horizon,title='SD-WACCM Ozone'    ; on '+sdate
...
; 
    ...
    map_set,/MOLLWEIDE,0,140,0,/noeras,color=0,/contin,/grid,/isotropic,/horizon,title='Trajectory Mapped Ozone'    ; on '+sdate
    contour,o3map2,xnd2,ynd,level=level,/cell_fill,c_color=col1,/noeras,color=0,/overplot,charsize=1.25,charthick=2
    map_set,/MOLLWEIDE,0,140,0,/noeras,color=0,/contin,/grid,/isotropic,/horizon
    contour,pv,x,alat,/overplot,/noeras,/follow,color=mcolor,levels=[4],thick=12,c_labels=0
    contour,sf,x,alat,level=sflevel,/overplot,/follow,color=mcolor,thick=5,c_label=0*sflevel
;loadct,13
...
;    oplot,xo3map(index),yo3map(index),psym=8,color=100	;(float(i+1)/11.)*mcolor
;endfor
;loadct,39
    ymnb=yorig(1) -cbaryoff
    ymxb=ymnb  +cbarydel
    set_viewport,xmn+0.1,xmx-0.1,ymnb,ymxb
    !type=2^2+2^3+2^6
...
;loadct,39
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim rdf_socrates_'+sdate+'_2pan+xys_'+isuff(nn)+'.ps -rotate -90 rdf_socrates_'+sdate+'_2pan+xys_'+isuff(nn)+'.jpg'
;      spawn,'/usr/bin/rm rdf_socrates_'+sdate+'_2pan+xys.ps'
    endif
endfor
end
