...
;!type=2^2+2^3
;loadct,39
;uyz=reform(u2(*,108,*))	; 90W
;sp2=sqrt(u2*u2+v2*v2)
;spyz=reform(sp2(*,108,*))
;markyz=reform(mark2(*,108,*))
;contour,uyz,alat,zprof,xrange=[-90,-30],yrange=[10,65],levels=10+10*findgen(20),/foll,xtitle='Latitude',ytitle='Altitude',c_color=(findgen(20)/21.)*255,thick=3,title='MERRA2 Zonal Wind at 90W Non-Oro GW case Vortex Edge',charsize=2,charthick=2,c_charsize=1.5
;;contour,uyz,alat,zprof,xrange=[-90,-30],levels=10+10*findgen(20),/foll,/overplot,color=mcolor
...
data = where(abs(airs_frac_day_all - scenetime/24.) le .25 and abs(lat-cips_lat_mean) lt 60. $		; more correct to use scenetime?
;data = where(airs_frac_day_all*24. gt 18.5 and airs_frac_day_all*24. le 19.5 and abs(lat-cips_lat_mean) lt 40. $
;data = where(airs_frac_day_all*24. gt 19.5 and airs_frac_day_all*24. le 23. and abs(lat-cips_lat_mean) lt 40. $
...
;stop
			...
			xyouts, .5,.98,plot_title,align=.5,charsize = 1.6
			loadct,0
	
;limit = [cips_lat_mean-20,cips_lon_mean-30,cips_lat_mean+20,cips_lon_mean+30]; Set plot limits based on the mean CIPS lat/lon
limit = [cips_lat_mean-12.5,cips_lon_mean-12.5,cips_lat_mean+12.5,cips_lon_mean+12.5]; Set plot limits based on the mean CIPS lat/lon
;limit = [-90,0,-30,360]; Set plot limits based on the mean CIPS lat/lon
 		...
 		col2 = [250,220,190,170,125,125,100,80,60,30,10,0]
...
;        		MAP_GRID, /LABEL, LATLAB=floor(cips_lon_mean/15.)*15.-20., LONLAB=floor(cips_lat_mean/10.)*10+17, LATDEL=5, LONDEL=10
 		        MAP_GRID, /LABEL, LATLAB=0., LONLAB=floor(cips_lat_mean/10.)*10+14, LATDEL=5, LONDEL=10, charsize=2.5
xyouts,-75,-40,'40S',/data,color=0,alignment=0.5,charsize=2.5
xyouts,-73.75,-45,'45S',/data,color=0,alignment=0.5,charsize=2.5
xyouts,-72,-50,'50S',/data,color=0,alignment=0.5,charsize=2.5
xyouts,-69.5,-55,'55S',/data,color=0,alignment=0.5,charsize=2.5
;		        MAP_GRID, /LABEL, LATLAB=floor(cips_lon_mean/15.)*15.-2., LONLAB=floor(cips_lat_mean/10.)*10+14, LATDEL=5, LONDEL=10, charsize=2.5
;
...
;loadct,39
sp=smooth(sp,3,/edge_truncate)
contour,sp,alon2,alat,/overplot,levels=5*findgen(20),color=0,c_labels=1+0*indgen(20),thick=3,c_charsize=1.75,c_charthick=2	;,c_color=((1.+findgen(6))/7.)*255.
contour,sp,alon2,alat,/overplot,levels=80+20*findgen(6),color=0,c_labels=[0,0,0,0,1,1],thick=3,c_charsize=1.75,c_charthick=2	;,c_color=((1.+findgen(6))/7.)*255.
contour,sp,alon2,alat,/overplot,levels=[160],color=0,c_labels=[0],thick=5,c_charsize=1.75,c_charthick=2	;,c_color=((1.+findgen(6))/7.)*255.
loadct,0
...
loadct,0
				...
				if airsplot eq 1 then position = [.52,0.02, .9,.09]
			...
			if AIRSplot eq 1 then begin
				...
				if cipsplot eq 1 then position = [.1,0.02, .48,.09]
