;
; save coincidences between CIPS and SABER. dxc and dtc are space and time criteria.
; find all coincidences with a single SABER profile. 
...
;	3) SABER lon,lat,uttime,ltime. Profiles of temperature and water vapor.
; lon(mprof,ncoinmax)
;
; then save medians and sigmas of the CIPS clusters
;
; CIPS level 4 data
; find coincidences between SABER and CIPS
; save daily files
; VLH 8/4/2009
;
@stddat
...
;sdir='/ptmp/harvey/SABER_data/Datfiles_SOSST/'
...
pthout='/aura7/harvey/CIPS_data/Datfiles_Save/'
...
; 
      if setplot eq 'ps' then begin
         ...
         device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
                /bold,/color,bits_per_pixel=8,/helvetica,filename='save_coin_saber_cips_'+sdate+'.ps'
         !p.charsize=1.25
         !p.thick=2
         !p.charthick=5
         !p.charthick=5
...
;
; restore SABER "TPZ" data on this day
;
...
;
      ...
      print,'read SABER on '+sdate
      nlv=n_elements(altitude)
      mtemp=temperature
      mpress=pressure
      mprof=n_elements(id)
;
; SABER lon,lat,uttime are 2-D. extract out 85 km for now
;
;      muttime=time
;      mlat=latitude
;      mlon=longitude
      muttime=reform(time(*,85))
      mlat=reform(latitude(*,85))
      mlon=reform(longitude(*,85))
;
; extract 0-120 km to match MLS
;
      mlev=121L
      altitude=reform(altitude(0:mlev-1L))
      mtemp=reform(mtemp(*,0:mlev-1L))
      mpress=reform(mpress(*,0:mlev-1L))
      mh2o=-99.+0.*mtemp
;
...
;
      ...
      mlttime=ltime
      mlday=lday
     print,'SABER LTTIME ',min(mlttime),max(mlttime)
     print,'SABER LDAY ',min(mlday),max(mlday)
...
;
      ...
      xcoinsaber=-99.+fltarr(mprof)
      ycoinsaber=-99.+fltarr(mprof)
      utcoinsaber=-99.+fltarr(mprof)
      ltcoinsaber=-99.+fltarr(mprof)
      tempcoinsaber=-99.+fltarr(mprof,mlev)
      h2ocoinsaber=-99.+fltarr(mprof,mlev)
      
      ncoinmax=8000L & dxc=400. & dtc=2.
      ...
      set_viewport,xmn,xmx,ymn,ymx
      !type=2^2+2^3
      if min(mlat) gt 0. then map_set,90,0,0,/ortho,/contin,/grid,title='SABER',color=0,limit=[50.,0.,80.,360.],/noerase
      if min(mlat) lt 0. then map_set,-90,0,0,/ortho,/contin,/grid,title='SABER',color=0,limit=[-50.,0.,-80.,360.],/noerase
      for ii=0L,mprof-1L do $
          oplot,[mlon(ii),mlon(ii)],[mlat(ii),mlat(ii)],psym=8,color=((mlttime(ii)-utmin)/(utmax-utmin))*mcolor	;,symsize=0.75
;
; find coincidences
; loop over SABER profiles
; 
      loadct,0
...
;         allindex=where(dist le dxc and clday eq lday0 and abs(clttime-ltime0) le dtc,ncoin)
          ...
          if cloudindex(0) ne -1L then begin
             ...
             xcoinsaber(i)=mlon(i)
             ycoinsaber(i)=mlat(i)
             utcoinsaber(i)=muttime(i)
             ltcoinsaber(i)=mlttime(i)
             tempcoinsaber(i,*)=mtemp(i,*)
             h2ocoinsaber(i,*)=mh2o(i,*)
          endif
      endfor
;
; remove saber profiles w/ no coincidences.  include cloud and no cloud.
;
      ...
      if mprof gt 0L then begin
         ...
         xcoinsaber=xcoinsaber(good)
         ycoinsaber=ycoinsaber(good)
         utcoinsaber=utcoinsaber(good)
         ltcoinsaber=ltcoinsaber(good)
         tempcoinsaber=tempcoinsaber(good,*)
         h2ocoinsaber=h2ocoinsaber(good,*)
;
; overplot coincidences
;
;  oplot,xcoincips,ycoincips,color=0,psym=8,symsize=0.2
;  oplot,xcoinsaber,ycoinsaber,color=0,psym=4
      endif
;
; plot color bar
;
...
;;
;; SAVE ALL CIPS COINCIDENCES WITH SABER ON THIS DAY
;;
      comment=strarr(3)
      comment(0)='First dimension is number of SABER profiles. Second dimension is either maximum number of coincidences allowed (for CIPS arrays) or number of altitudes (for SABER arrays).'
      comment(1)='Coincidence Criteria = '+sdxc+' km and '+sdtc+' hours'
      comment(2)='Water Vapor is in ppmv.'
      save,file=pthout+'cips_saber_coins_'+sdate+'_all.sav',comment,ncoins,nclouds,$
           xcoinsaber,ycoinsaber,utcoinsaber,ltcoinsaber,tempcoinsaber,h2ocoinsaber,ldoy,$
...
;
             if max(pts) gt 360. and min(pts) lt 10. then begin
                if xcoinsaber(iprof) lt 180. then begin
                   index=where(pts gt 360.)
                   pts(index)=pts(index)-360.
                endif
                if xcoinsaber(iprof) gt 180. then begin
                   index=where(pts lt 10.)
                   pts(index)=pts(index)+360.
                endif
                print,'GM logic used ',xcoinsaber(iprof),median(pts)
           endif
...
;
; SAVE MEDIAN and SIGMA of clusters of CIPS CLOUD COINCIDENCES WITH SABER ON THIS DAY
;
    comment=strarr(3)
    comment(0)='First dimension is number of SABER profiles. Second dimension is either maximum number of coincidences allowed (for CIPS arrays) or number of altitudes (for SABER arrays).'
    comment(1)='Coincidence Criteria = '+sdxc+' km and '+sdtc+' hours'
    comment(2)='Water Vapor is in ppmv.'
    save,file=pthout+'cips_saber_coins_'+sdate+'_median.sav',comment,ncoins,nclouds,$
         xcoinsaber,ycoinsaber,utcoinsaber,ltcoinsaber,tempcoinsaber,h2ocoinsaber,ldoy,$
...
;
    ...
    plot,x,x,xrange=[0.,360.],yrange=[0.,360.],charsize=1.2,$
         ytitle='SABER Longitude',xtitle='CIPS Median Longitude',title='Scatter Plot',/noerase,color=0
;
...
;
    ...
    xcoinsaber=xcoinsaber(good)
    result=correlate(xcoincips,xcoinsaber)
    r=result(0)
...
;
    ...
    y2=histogram(xcoinsaber,min=min(x),max=max(x),binsize=x(1)-x(0))/(1.*mprof)
    y2=smooth(y2,3)     ; SABER PDF
    ...
    xyouts,0.8*max(x),.8*ymax,'SABER',/data,charsize=1.2,color=0
    xyouts,0.8*max(x),.9*ymax,'CIPS',/data,charsize=1.2,color=0
;ks_stats,xcoincips,xcoinsaber,kstest,cprob
...
;saberbar=total(xcoinsaber)/n_elements(xcoinsaber)
;xcoincips=xcoincips-cipsbar+saberbar
;ks_stats,xcoincips,xcoinsaber,kstest,cprob
...
;       100.*cprob),/remove_all)+'%',/data,color=0
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim save_coin_saber_cips_'+sdate+'.ps -rotate -90 save_coin_saber_cips_'+sdate+'.jpg'
       spawn,'rm -f save_coin_saber_cips_'+sdate+'.ps'
    endif
goto,jump
end
