...
RADEA=6.37E6
;
; read dmps_20100805.nc
;
idir='/Users/harvey/Brakebusch/'
ifile=idir+'dmps_20100805.nc'
ofile=idir+'mark_20100805.sav'
...
;
            arg1 = (UGRD(IP1,LAT,K)-UGRD(IM1,LAT,K))/DX1 $
                  - VGRD(I,LAT,K)*TAN(ALAT(LAT)*DTR)/RADEA
            arg2 = (VGRD(IP1,LAT,K)-VGRD(IM1,LAT,K))/DX1 $
                  + UGRD(I,LAT,K)*TAN(ALAT(LAT)*DTR)/RADEA
            DVDY = (VGRD(I,JP1,K)-VGRD(I,JM1,K))/DY1
            DUDY = (UGRD(I,JP1,K)-UGRD(I,JM1,K))/DY1
            if abs(arg1) gt 1.e12 or abs(arg2) gt 1.e12 then QDFGRD(I,LAT,K) = 1.e12
            if abs(arg1) lt 1.e12 and abs(arg2) lt 1.e12 then begin
               qtemp=(0.5*(arg1*arg1+DVDY*DVDY)+arg2*DUDY)*RADEA*RADEA
               if qtemp ge 0.0 then QDFGRD(I,LAT,K) = sqrt(qtemp)
               if qtemp lt 0.0 then QDFGRD(I,LAT,K) = -sqrt(-qtemp)
            endif
...
for k= 0, NTH-1 do begin
    ...
    for I = 0, nc-1 do begin
       xlst = xlst + QDFGRD(i,nr-2,k)/float(nc)
       frst = frst + QDFGRD(i,1,k)/float(nc)
    endfor
    for I = 0, nc-1 do begin
       QDFGRD(i,nr-1,k) = xlst
       QDFGRD(i,0,k) = frst
    endfor
;
; check
...
; write marker file
;
save,filename=ofile,alon,alat,thlev,markgrd
