...
;
; read one CIPS IDL save file of all days of cloud albedo binned every 5 degrees latitude
...
;
    if setplot eq 'ps' then begin
       ...
       device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
              /bold,/color,bits_per_pixel=8,/helvetica,filename='timeseries_cipsl4_cloudalb_all_lat.ps'
       !p.charsize=1.25
       !p.thick=2
       !p.charthick=5
       !p.charthick=5
...
;   cloud_freq_avg_lat2=smooth(reform(cloud_freq_avg_all(*,ilat2)),3)       ; fltarr(kday) at ilat2
    ...
    plot,findgen(ndoy),cloud_freq_max_lat2,ytitle='Cloud Albedo',xrange=[imin,imax],$
         xticks=nxticks,xtickname=xlab,xtickv=float(xlab),charsize=1.5,color=0,/nodata,$
         title='CIPS Level 4 V3.20',yrange=[0.,30.]
    endif
...
;   cloud_freq_avg_lat=smooth(reform(cloud_freq_avg_all(*,ilat)),3)     ; fltarr(kday) at ilat
    ...
    xlab=['-40','-20','0','20','40','60','80']
    nxticks=n_elements(xtickname)
    plot,findgen(ndoy),cloud_freq_max_lat,ytitle='Cloud Albedo',xrange=[imin,imax],$
         xticks=nxticks,xtickname=xlab,xtickv=float(xlab),charsize=1.5,color=0,/nodata,$
         yrange=[0.,30.],xtitle='Days from Solstice'
    endif
    syear=strmid(strcompress(sdate_all,/remove_all),0,4)
;
    shsol=355.
...
;
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim timeseries_cipsl4_cloudalb_all_lat.ps -rotate -90 timeseries_cipsl4_cloudalb_all_lat.jpg'
    endif
