...
result=file_search('cips_l2_medians_vs_sza_south_'+syear+'.sav')
if result(0) eq '' then goto,skipyear
restore,'cips_l2_medians_vs_sza_south_'+syear+'.sav
print,'restored cips_l2_medians_vs_sza_south_'+syear+'.sav'
...
season=(iyear-2007L)+0L*DFS_ALL		; integer value per season to plot around New Year instead of constant year
...
season=[season,(iyear-2007L)+0L*DFS_ALL]
endif
skipyear:
endfor
...
erase
      if setplot eq 'ps' then begin
         ...
         device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
                /bold,/color,bits_per_pixel=8,/helvetica,filename='scatter-6pan_cips_l4_v4.20_v5.10_rawfiles_south_allyears.ps'
         !p.charsize=1.2
         !p.thick=2
         !p.charthick=5
         !y.thick=2
...
for j=0L,nbin-1L do xx2d(*,j)=season	; 0=2007/2008 season, 1=2008/2009 season, ... 8=2015/2016 season, missing v4 for 2014/2015 and discontinued prior to 2016/2017
...
for iyear=year1,year2 do begin
    index=where(xx2d eq (iyear-year1) and y2d le 70. and med51 gt 0. and med20 gt 0.) ; DES
    if index(0) ne -1L then oplot,med20(index),med51(index),color=250,psym=8,symsize=0.5
    ...
    index=where(xx2d eq (iyear-year1) and y2d gt 70. and med51 gt 0. and med20 gt 0.)   ; ASC
    if index(0) ne -1L then oplot,med20(index),med51(index),color=90,psym=8,symsize=0.5
    result=linfit(med20(index),med51(index),yfit=linevals)
    b=result(0)
    slope=result(1)
    y=slope*x+b
...
;   index=where(xx2d eq iyear and med51 gt 0. and med20 gt 0.)	; DES
    index=where(xx2d eq (iyear-year1) and med51 gt 0. and med20 gt 0.)	; DES
    if index(0) ne -1L then oplot,med20(index),med51(index),color=col1(iyear-year1),psym=8,symsize=0.5
    result=linfit(med20(index),med51(index),yfit=linevals)
    b=result(0)
    slope=result(1)
    y=slope*x+b
...
plot,med20(index),med51(index),color=0,psym=8,symsize=1,ytitle='5.1 RAD',xtitle='4.2 RAD',xrange=[20,100],yrange=[20,100],charsize=1.2,charthick=2,/noeras,/nodata
for iyear=year1,year2 do begin
    index=where(xx2d eq (iyear-year1) and med51 gt 0. and med20 gt 0.)
    if index(0) ne -1L then oplot,med20(index),med51(index),color=col1(iyear-year1),psym=8,symsize=0.5
    result=linfit(med20(index),med51(index),yfit=linevals)
    b=result(0)
    slope=result(1)
...
oplot,level,level,thick=2,color=0
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim scatter-6pan_cips_l4_v4.20_v5.10_rawfiles_south_allyears.ps -rotate -90 scatter-6pan_cips_l4_v4.20_v5.10_rawfiles_south_allyears.jpg'
;      spawn,'rm -f scatter-6pan_cips_l4_v4.20_v5.10_rawfiles_south_allyears.ps'
    endif
end
