...
;             print,'read variable ',result.name
          ...
          latitude_orig=latitude				;save original lats to determine asc/desc. Ascending latitudes are gt 90 or lt -90. Descending lats are btw -90 and 90.
...
; 
          HEAP_GC
;
; remove data with infinite albedo values
;
          good=WHERE(FINITE(CLD_ALBEDO) EQ 1,ngood)	
...
;            ut_date=ut_date(good)		; v4.2 ut_date=one number YYYYDOY
             ...
             SZA=ZENITH_ANGLE_RAY_PEAK(good)
             CLOUD_INDEX=CLOUD_PRESENCE_MAP(good)
             QUALITY_FLAGS=QUALITY_FLAGS(good)
             ALB=CLD_ALBEDO(good)
             ALB_ERR=CLD_ALBEDO_UNC(good)
...
;
          if kcount eq 0L then begin
             ...
             QUALITY_FLAGS_all=QUALITY_FLAGS
             ALB_all=ALB
             ALB_ERR_all=ALB_ERR
             RAD_all=RAD
             RAD_ERR_all=RAD_ERR
          ...
          if kcount gt 0L then begin
             ...
             QUALITY_FLAGS_all=[QUALITY_FLAGS_all,QUALITY_FLAGS]
             ALB_all=[ALB_all,ALB]
             ...
             ICD_all=[ICD_all,ICD]
             NLAYERS_all=[NLAYERS_all,NLAYERS]
          endif
          kcount=1
skiporbit:
