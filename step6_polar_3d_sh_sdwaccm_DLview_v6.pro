...
; save postscript version
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='Antarctic_vortex/'+sdate+'_sdwaccm_v6_3D_antarctic_vortex.ps'
       device,/color
...
;          contour,temp,xcs,ycs,levels=[180.],color=0.1*mcolor,/T3D,zvalue=nz,thick=2,min_value=-1.e15
           if th(lev) gt 2000. then contour,temp,xcs,ycs,levels=[120,130,140],c_color=[20,50,70],/T3D,zvalue=nz,thick=3,min_value=-1.e15
...
;              if max(y2d(index)) ge -2.5 then goto,jump1
               ...
               index=where(finite(tmp) and y2d le -13.75 and dum eq -1.0*(ihigh+1))
;               index=where(finite(tmp) and y2d lt 0. and dum eq -1.0*(ihigh+1))
                if index(0) eq -1L then goto,jump1
                oplot,xcs(index),ycs(index),psym=8,color=0,/T3D,zvalue=nz,symsize=1
...
;   xyouts,.78,.85,'Pressure (hPa)',charsize=2,/normal,color=0,charthick=2
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim Antarctic_vortex/'+sdate+'_sdwaccm_v6_3D_antarctic_vortex.ps -rotate -90 Antarctic_vortex/'+sdate+'_sdwaccm_v6_3D_antarctic_vortex.jpg'
       spawn,'rm -f Antarctic_vortex/'+sdate+'_sdwaccm_v6_3D_antarctic_vortex.ps'
    endif
