;
; SH post-PMC season Mar 1 to Mar 15 with the solstice on day 355
...
;
plotit:
;restore,'yt_sh_post_'+syear+'_QF01_ALB1.sav'	;,asc_freq,des_freq,dfs,latbin,syear,szamax_asc,szamin_asc,albmax_asc,albmin_asc,szamax_des,szamin_des,albmax_des,albmin_des
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='yt_cloud_freq_v4.2_'+syear+'_sh_post.ps'
   !p.charsize=1.5
   !p.thick=1.5
   !p.charthick=5
   !p.charthick=5
...
erase
xyouts,.1,.95,'CIPS Version 4.20 Cloud Frequency 1-15 Mar',/normal,color=0,charsize=1.75
...
contour,asc_freq,dfs,latbin,c_color=col1,/fill,xtitle='DFS '+syear,ytitle='Latitude',title='Ascending Node',xrange=[min(dfs),max(dfs)],yrange=[-90,-30],/noeras,levels=level,color=0
contour,asc_freq,dfs,latbin,color=mcolor,/follow,/overplot,/noeras,levels=level,c_labels=0*indgen(nlvls)
xyouts,dfs(1),-35.,'SZA '+sszamin_asc+' - '+sszamax_asc,/data,color=0,charsize=1.5
xyouts,dfs(1),-45.,'Alb '+salbmin_asc+' - '+salbmax_asc,/data,color=0,charsize=1.5
xyouts,dfs(12),-35.,'Max',/data,color=0,charsize=1.5
xyouts,dfs(12),-45.,string(FORMAT='(f5.2)',max(asc_freq)),/data,color=0,charsize=1.5
...
ymx=yorig(1)+ylen
set_viewport,xmn,xmx,ymn,ymx
contour,des_freq,dfs,latbin,c_color=col1,/fill,xtitle='DFS '+syear,ytitle='Latitude',title='Descending Node',xrange=[min(dfs),max(dfs)],yrange=[-90,-30],/noeras,levels=level,color=0
contour,des_freq,dfs,latbin,color=mcolor,/follow,/overplot,/noeras,levels=level,c_labels=0*indgen(nlvls)
xyouts,dfs(1),-35.,'SZA '+sszamin_des+' - '+sszamax_des,/data,color=0,charsize=1.5
xyouts,dfs(1),-45.,'Alb '+salbmin_des+' - '+salbmax_des,/data,color=0,charsize=1.5
xyouts,dfs(12),-35.,'Max',/data,color=0,charsize=1.5
xyouts,dfs(12),-45.,string(FORMAT='(f5.2)',max(des_freq)),/data,color=0,charsize=1.5
...
save,file='yt_sh_post_'+syear+'_QF01_ALB1.sav',asc_freq,des_freq,dfs,latbin,syear,$
...
; Close PostScript file and return control to X-windows
 ...
 if setplot eq 'ps' then begin
    device, /close
    spawn,'convert -trim yt_cloud_freq_v4.2_'+syear+'_sh_post.ps -rotate -90 '+$
                        'yt_cloud_freq_v4.2_'+syear+'_sh_post.jpg'
    spawn,'rm -f yt_cloud_freq_v4.2_'+syear+'_sh_post.ps'
 endif
end
