...
if n_elements(index) gt 1 then areasig_allyears(iyear)=stdev(tmp(index))
...
plot,lyear,area_allyears,yrange=[0,20],color=0,charsize=2,charthick=2,ytitle=sth+' Vortex Area (%)',xrange=[min(lyear),max(lyear)],psym=8,/noeras,symsize=1.5
