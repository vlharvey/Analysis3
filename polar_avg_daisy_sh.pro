;
; read CIPS level 3 data and plot average daisy for time range. SH. DFS=Dec 21
;
...
;
; adjust doy and increment year at Jan 1
;
    kday=iday
    syear0=syear
    if iyear mod 4 eq 0 and iday gt 366 then begin	; leap years
       kday=iday-366.
       syear0=string(FORMAT='(I4)',iyear+1)
    endif
    if iyear mod 4 ne 0 and iday gt 365 then begin	; non leap years
       kday=iday-365.
       syear0=string(FORMAT='(I4)',iyear+1)
    endif
...
;
    spawn,'ls '+pth+syear0+'-'+sday+'_v04.20_r05.nc.gz',fname
    if fname(0) eq '' then goto,jumpday
    print,fname
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='polar_avg_daisy_sh_'+syear+'.ps'
   !p.charsize=1.5
   !p.thick=1.5
   !p.charthick=5
   !p.charthick=5
...
set_viewport,xmn,xmx,ymn,ymx
; LIMIT has the form [Latmin, Lonmin, Latmax, Lonmax]
map_set,-90,0,0,/ortho,/contin,/grid,color=0,/noeras,title='CIPS L3 V4.20 on -10 to +30 DFS in '+syear,limit=[-90, 0, -50, 360]
level=[.1,1.+findgen(nlvls-1)]
...
contour,smooth(alb_avg,9,/NaN),alon,alat,/follow,levels=[10.,20.,30.],/noeras,/overplot,color=mcolor,c_labels=[1,1,1,1]
imin=min(level)
imax=max(level)
ymnb=ymn -cbaryoff
ymxb=ymnb+cbarydel
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim polar_avg_daisy_sh_'+syear+'.ps -rotate -90 polar_avg_daisy_sh_'+syear+'.jpg'
;  spawn,'rm -f polar_avg_daisy_sh_'+syear+'.ps'
endif
end
