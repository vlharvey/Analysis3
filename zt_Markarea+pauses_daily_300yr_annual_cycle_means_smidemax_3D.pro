...
;
    if setplot eq 'ps' then begin
       ...
       device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
              /bold,/color,bits_per_pixel=8,/helvetica,filename='zt_TMarkarea+pauses_NH_SH_smidemax.ps'
       !p.charsize=1.1
       !p.thick=2
       !p.charthick=2
       !y.thick=2
...
tlevel=[130+5*findgen(27),280,300,320,350,400,450,500]
nlvls1=n_elements(tlevel)
col2=(findgen(nlvls1)/float(nlvls1))*mcolor
;
; superimpose stratopause and mesopause
;
...
contour,tnhz,findgen(ndays),altitude,/noera,/fill,color=0,c_color=col2,levels=tlevel,xrange=[0,ndays-1],yrange=[30,125],ytitle='Altitude (km)',charsize=1.25,charthick=2,title='NH Temp',$
        xticks=nxticks0-1,xtickname=xlabs0,xtickv=xindex0
contour,tnhz,findgen(ndays),altitude,/noera,/foll,color=0,levels=[230,250,270,300,350,400,450],/overplot
contour,tnhz,findgen(ndays),altitude,/noera,/foll,color=mcolor,levels=[130,150,170,190,210],/overplot
...
ymx=yorig(1)+ylen
set_viewport,xmn,xmx,ymn,ymx
level=50+5*findgen(8)
nlvls=n_elements(level)
col1=reverse((findgen(nlvls)/float(nlvls))*mcolor)
contour,vareamean_ztnh,findgen(ndays),znh,/noera,/cell_fill,color=0,c_color=col1,levels=level,thick=10,xrange=[0,ndays-1],yrange=[30,125],charsize=1.25,charthick=2,title='NH Vortex',$
        xticks=nxticks0-1,xtickname=xlabs0,xtickv=xindex0
contour,vareamean_ztnh,findgen(ndays),znh,/noera,/foll,color=mcolor,levels=[75],/overplot
...
ymx=yorig(2)+ylen
set_viewport,xmn,xmx,ymn,ymx
contour,hareamean_ztnh,findgen(ndays),znh,/noera,/cell_fill,color=0,c_color=col1,levels=level,thick=10,xrange=[0,ndays-1],yrange=[30,125],charsize=1.25,charthick=2,title='NH Highs',$
        xticks=nxticks0-1,xtickname=xlabs0,xtickv=xindex0
contour,hareamean_ztnh,findgen(ndays),znh,/noera,/foll,color=mcolor,levels=[75],/overplot
...
contour,tshz,findgen(ndays),altitude,/noera,/fill,color=0,c_color=col2,levels=tlevel,xrange=[0,ndays-1],yrange=[30,125],ytitle='Altitude (km)',charsize=1.25,charthick=2,title='SH Temp',$
        xticks=nxticks-1,xtickname=xlabs,xtickv=xindex
contour,tshz,findgen(ndays),altitude,/noera,/foll,color=0,levels=[230,250,270,300,350,400,450],/overplot
contour,tshz,findgen(ndays),altitude,/noera,/foll,color=mcolor,levels=[130,150,170,190,210],/overplot
...
ymx=yorig(4)+ylen
set_viewport,xmn,xmx,ymn,ymx
contour,vareamean_ztsh,findgen(ndays),zsh,/noera,/cell_fill,color=0,c_color=col1,levels=level,thick=10,xrange=[0,ndays-1],yrange=[30,125],charsize=1.25,charthick=2,title='SH Vortex',$
            xticks=nxticks-1,xtickname=xlabs,xtickv=xindex
contour,vareamean_ztsh,findgen(ndays),zsh,/noera,/foll,color=mcolor,levels=[75],/overplot
loadct,0
oplot,findgen(ndays),zstratsh,color=0,psym=8
oplot,findgen(ndays),zmesosh,color=200,psym=8
loadct,39
...
ymx=yorig(5)+ylen
set_viewport,xmn,xmx,ymn,ymx
contour,hareamean_ztsh,findgen(ndays),zsh,/noera,/cell_fill,color=0,c_color=col1,levels=level,thick=10,xrange=[0,ndays-1],yrange=[30,125],charsize=1.25,charthick=2,title='SH Highs',$
            xticks=nxticks-1,xtickname=xlabs,xtickv=xindex
contour,hareamean_ztsh,findgen(ndays),zsh,/noera,/foll,color=mcolor,levels=[75],/overplot
...
xyouts,(x0+x1)/4.,y0-0.03,'Polar Cap Temp (K)',color=0,charsize=1.25,charthick=2,/normal
slab=strcompress(string(format='(i3)',long(tlevel)),/remove_all)
...
plot,[0,0],[0,0],yrange=[0,10],xrange=[0,1],/noeras,xticks=nlvls-1L,$
        position = [x0,y0,x1,y1],xstyle=1,xtickname=slab,/nodata,color=0
xyouts,(x0+x1)/3.+0.04,y0-0.03,'Area Equivalent Latitude',color=0,charsize=1.25,charthick=2,/normal
...
;
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim zt_TMarkarea+pauses_NH_SH_smidemax.ps -rotate -90 zt_TMarkarea+pauses_NH_SH_smidemax.jpg
;      spawn,'rm -f zt_TMarkarea+pauses_NH_SH_smidemax.ps'
    endif
