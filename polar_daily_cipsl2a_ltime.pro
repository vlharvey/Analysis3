...
;
    if setplot eq 'ps' then begin
       ...
       device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
              /bold,/color,bits_per_pixel=8,/helvetica,filename='polar_daily_cipsl2a_ltime_'+sdate+'.ps'
       !p.charsize=1.25
       !p.thick=2
       !p.charthick=5
       !p.charthick=5
...
;print,min(ut_time),max(ut_time),min(longitude),max(longitude)
          ALB = (*data[0].albedo)				;Cloud albedo in Garys (10^-6 sr^-1) i.e., alb x 1.e6
;
; compute local time from UT time and longitude
;
LTIME=0.*ut_time
   X=WHERE(longitude LE 180.,NX)
   IF NX GT 0 THEN LTIME(X)=ut_time(X)+longitude(X)*24./360.
   X=WHERE(longitude GT 180.,NX)
   IF NX GT 0 THEN LTIME(X)=ut_time(X)-longitude(X)*24./360.
ltime=abs(ltime)
print,'ltime ',min(ltime),max(ltime)
;
; free memory
; 
          HEAP_GC
...
index=where(ltime gt 0. and ltime le 1.)
if index(0) ne -1L then oplot,longitude(index),latitude(index),psym=3,color=mcolor*.1
index=where(ltime gt 1. and ltime le 3.)
if index(0) ne -1L then oplot,longitude(index),latitude(index),psym=3,color=mcolor*.2
index=where(ltime gt 3. and ltime le 5.)
if index(0) ne -1L then oplot,longitude(index),latitude(index),psym=3,color=mcolor*.3
index=where(ltime gt 5. and ltime le 7.)
if index(0) ne -1L then oplot,longitude(index),latitude(index),psym=3,color=mcolor*.4
index=where(ltime gt 7. and ltime le 9.)
if index(0) ne -1L then oplot,longitude(index),latitude(index),psym=3,color=mcolor*.5
index=where(ltime gt 9. and ltime le 11.)
if index(0) ne -1L then oplot,longitude(index),latitude(index),psym=3,color=mcolor*.6
index=where(ltime gt 11. and ltime le 13.)
if index(0) ne -1L then oplot,longitude(index),latitude(index),psym=3,color=mcolor*.7
index=where(ltime gt 13. and ltime le 15.)
if index(0) ne -1L then oplot,longitude(index),latitude(index),psym=3,color=mcolor*.8
index=where(ltime gt 15.)
if index(0) ne -1L then oplot,longitude(index),latitude(index),psym=3,color=mcolor*.9

endfor	; loop over orbits
slevel=['0','1','3','5','7','9','11','13','15','>15']
...
; Close PostScript file and return control to X-windows
     ...
     if setplot eq 'ps' then begin
        device, /close
;       spawn,'convert -trim polar_daily_cipsl2a_ltime_'+sdate+'.ps -rotate -90 '+$
;                           'polar_daily_cipsl2a_ltime_'+sdate+'.jpg'
;       spawn,'/usr/bin/rm polar_daily_cipsl2a_ltime_'+sdate+'.ps'
     endif
