...
;
      CONAFPL_all=fltarr(nr,nl,nyears)
;
...
;
      for j=0L,nr-1L do begin
          for k=0L,nl-1L do begin
              CONAFPL_avg(j,k,icount)=mean(CONAFPL_all(j,k,*))
              CONAFPL_sig(j,k,icount)=stdev(CONAFPL_all(j,k,*))
          endfor
      endfor
jump2count:
      icount=icount+1L
...
print,ofile
save,file=ofile,lat,lev,sdate_all,CONAFPL_avg,CONAFPL_sig
end
