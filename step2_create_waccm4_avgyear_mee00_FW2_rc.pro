...
;
      if icount eq 0L then begin
         ...
         VSTAR00FPL_sig=fltarr(nr,nl,kday)
         WSTAR00FPL_sig=fltarr(nr,nl,kday)
...
;
      VSTAR00FPL_all=fltarr(nr,nl,nyears)
      WSTAR00FPL_all=fltarr(nr,nl,nyears)
;
...
;         print,'restored '+ifiles(iyear)
          VSTAR00FPL_all(*,*,iyear)=VSTAR
          WSTAR00FPL_all(*,*,iyear)=WSTAR
      endfor
;
...
;
      for j=0L,nr-1L do begin
          for k=0L,nl-1L do begin
              VSTAR00FPL_avg(j,k,icount)=mean(VSTAR00FPL_all(j,k,*))
              WSTAR00FPL_avg(j,k,icount)=mean(WSTAR00FPL_all(j,k,*))

              VSTAR00FPL_sig(j,k,icount)=stdev(VSTAR00FPL_all(j,k,*))
              WSTAR00FPL_sig(j,k,icount)=stdev(WSTAR00FPL_all(j,k,*))
          endfor
      endfor
jump2count:
      icount=icount+1L
...
print,ofile
save,file=ofile,lat,lev,sdate_all,VSTAR00FPL_avg,WSTAR00FPL_avg,VSTAR00FPL_sig,WSTAR00FPL_sig
end
