;
; read CIPS level 4 data and store daily average radius, albedo, IWC
...
;print,min(iwc(bad))
          ENDIF
;
; check number of good phase albedos.  do they have to be positive?
;
;for i=0L,xdim-1L do begin
;for j=0L,ydim-1L do begin
;    if alb(i,j) ne -99. then begin
;       phase_albs=reform(CLD_PHASE_ALBEDO(i,j,*))
;       index=where(finite(phase_albs) eq 1. and phase_albs ne 0.,nn)
;       if nn gt 0L then print,nn
;    endif
;endfor
;endfor
;stop
...
;   map_set,90,0,0,/ortho,/contin,/grid,title=sdate
   plot,alb,rad,psym=1,xrange=[0.,20.],yrange=[0.,100]
endif
oplot,alb,rad,psym=1
;amin=lim
;amax=20.
...
;
; need to retain all raw orbit data for averaging at the end. add to these
;
             ALB_clouds=-99.
             IWC_clouds=-99.
             lat_clouds=-99.
...
;erase
;!type=2^2+2^3
;plot,latbin,latbin,psym=2,/nodata,title=sdate,yrange=[0.,100.],$
;     ytitle='All Radii',xtitle='Latitude',xrange=[50.,90.],color=0
;endif
;
...
;
; for clouds points, retain all albedos, iwc, radii, and latitudes
;
             ...
             if x(0) ne -1L then begin
                cloud_points(ilat,iorbit)=float(nx)
                ALB_clouds=[alb_clouds,alb(x)]
                IWC_clouds=[iwc_clouds,iwc(x)]
                rad_clouds=[rad_clouds,rad(x)]
                lat_clouds=[lat_clouds,latitude(x)]
             endif
             if float(nx) gt float(npts) then stop,'more clouds than data points'
         endfor	; loop over latitude bins
      ENDFOR	; LOOP OVER ORBITS
;
; average in each latitude bin. require at least 3 cloud points per bin.
;
      FOR ilat=0,NLAT-1 DO BEGIN
          index=WHERE(LAT_clouds GT latbin(ilat)-yinc AND LAT_clouds LE latbin(ilat)+yinc,nx)
          if nx ge 3L then begin
             ALB_avg(ilat)=mean(ALB_clouds(index))
             IWC_avg(ilat)=mean(iwc_clouds(index))
             RAD_avg(ilat)=mean(rad_clouds(index))
             tot_avg(ilat)=nx
          endif
      endfor
;index=where(tot_avg ne 0.)
;if index(0) ne -1L then begin
;   plot,latbin(index),alb_avg(index),psym=0,color=0
;endif
;erase
;plot,ALB_clouds,rad_clouds,psym=1,xrange=[0.,20.],yrange=[0.,100]
;stop
;plot,alb,rad,psym=1,xrange=[0.,20.],yrange=[0.,100]
;oplot,alb,rad,psym=1
;stop
...
;
      ...
      print,ofile
      SAVE,file=ofile,latbin,cloud_points,total_points,norbit,alb_avg,iwc_avg,rad_avg,tot_avg
goto,jump
end
