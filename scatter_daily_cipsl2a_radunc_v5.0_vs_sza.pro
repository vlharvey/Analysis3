...
;
    if setplot eq 'ps' then begin
       ...
       device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
              /bold,/color,bits_per_pixel=8,/helvetica,filename='scatter_daily_cipsl2a_radunc_v5.0_vs_sza_'+sdate+'.ps'
       !p.charsize=1.25
       !p.thick=2
       !p.charthick=5
       !p.charthick=5
   ...
   !type=2^2+2^3
   plot,sza,rad/rad_err,xrange=[50,120],yrange=[0,100],psym=3,color=0,xtitle='SZA',ytitle='Radius/UNC',charsize=2,charthick=2
...
oplot,sza,rad/rad_err,psym=8,color=col1(iorbit)
...
xyouts,52,85,'ALBERR Range ('+strcompress(min(alb_err_all),/r)+' to '+strcompress(long(max(alb_err_all)),/r)+')',/data,color=0,charsize=1.2,charthick=2
xyouts,52,80,'RAD Range ('+strcompress(long(rmin),/r)+' to '+strcompress(long(rmax),/r)+')',/data,color=0,charsize=1.2,charthick=2
xyouts,52,75,'RADERR Range ('+strcompress(long(min(rad_err_all)),/r)+' to '+strcompress(long(max(rad_err_all)),/r)+')',/data,color=0,charsize=1.2,charthick=2
index=where(CLOUD_PRESENCE_MAP_ALL eq 1.)
if index(0) ne -1L then oplot,sza_all(index),rad_all(index)/rad_err_all(index),psym=8,color=250,symsize=0.5
loadct,0
...
; Close PostScript file and return control to X-windows
     ...
     if setplot eq 'ps' then begin
        device, /close
        spawn,'convert -trim scatter_daily_cipsl2a_radunc_v5.0_vs_sza_'+sdate+'.ps -rotate -90 scatter_daily_cipsl2a_radunc_v5.0_vs_sza_'+sdate+'.jpg'
        spawn,'rm -f scatter_daily_cipsl2a_radunc_v5.0_vs_sza_'+sdate+'.ps'
     endif
