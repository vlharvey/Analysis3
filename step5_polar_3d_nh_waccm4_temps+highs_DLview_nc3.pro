;
; WACCM4 3D Arctic vortex and anticyclones
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='3D_meeFW2_figs/polar_3d_nh_waccm4_'+sdate+'_DL+highs_nc3.ps'
   device,/color
...
dx=x2d(1,0)-x2d(0,0)
        ...
        dum(nc,*)=dum(0,*)
               ...
               if min(y2d(index)) le 1. then goto,jump1		; exclude more tropical anticyclones
...
;if bad(0) ne -1L then dum(bad)=0./0.
               ...
               index=where(y2d ge 1. and dum eq -1.0*(ihigh+1))
...
;       xyouts,.08,nz4,thlevs(kk),color=0,/normal,charsize=2,charthick=2
    ...
    !psym=0
    xyouts,0.2,0.9,'Arctic Vortex in WACCM-4',/normal,charsize=3,color=0,charthick=2
...
;   xyouts,.08,.83,'Theta (K)',charsize=2,/normal,color=0,charthick=2
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim 3D_meeFW2_figs/polar_3d_nh_waccm4_'+sdate+'_DL+highs_nc3.ps -rotate -90 '+$
                           '3D_meeFW2_figs/polar_3d_nh_waccm4_'+sdate+'_DL+highs_nc3.jpg'
       spawn,'rm -f 3D_meeFW2_figs/polar_3d_nh_waccm4_'+sdate+'_DL+highs_nc3.ps'
    endif
