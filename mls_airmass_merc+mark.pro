...
hfile=[$
'ukmo_mls_mar_2005_3d_back.airm']
...
; plot UKMO quantity in top panel
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='mls_o3_'+stheta+$
...
;    o3min=0.
    ...
    xyouts,.35,.8,'MLS Ozone '+stheta,/normal,charsize=2
    ...
    contour,mark,x,alat,/overplot,levels=[-0.1],/noeras,color=0,c_labels=[0],thick=5
    loadct,0
    contour,mark,x,alat,/overplot,levels=[0.1],/noeras,color=mcolor,c_labels=[0],thick=8
    loadct,38
...
; color bar for Ozone data
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim mls_o3_'+stheta+'_'+stime+'+mark_merc.ps -rotate -90 '+$
             ' mls_o3_'+stheta+'_'+stime+'+mark_merc.jpg'
       spawn,'/usr/bin/rm mls_o3_'+stheta+'_'+stime+'+mark_merc.ps'
    endif
