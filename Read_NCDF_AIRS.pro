...
for ifile = 0, nfiles - 1L do begin
	...
	if iloop eq 1 then begin
		...
		level1 = [0,.02,.04,.08,.1,.15,.2,.25,.3,.35,.4,.5,1]
	endif
	if iloop eq 2 then begin
		...
		level1 = [0,.02,.04,.08,.1,.15,.2,.25,.3,.35,.4,.5,1,5]
	endif
	lats = reform(lat[x])
	lons = reform(lon[x])
	help, var15
...
contour, var15, lons,lats, /overplot, levels=level1, /cell_fill, c_color = col1,/noerase,/IRREGULAR,$;
...
for i = 0, n_elements(lonlab) - 1L do xyouts, lonlab,floor(cips_lat/10.)*10,  string(lonlab,format = '(i4.3)'),align = .5
	...
	loadct,39
			...
			position = [.1,0.09, .9,.17],xstyle=1,xtickname=slab,xtitle= 'T Variance (K^2)'
