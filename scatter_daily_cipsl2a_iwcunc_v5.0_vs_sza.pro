;
; read CIPS level 2a version 5.00 data and plot IWC/uncertainty for all orbits each day
;
loadct,0
mcolor=!p.color
...
;             print,'read variable ',result.name
          ...
          rad=PARTICLE_RADIUS
          rad_err=PARTICLE_RADIUS_UNC
          iwc=ICE_WATER_CONTENT
          iwc_err=ICE_WATER_CONTENT_UNC
;
; free memory
; 
          HEAP_GC
...
;
          IF NGOOD GT 0 THEN BEGIN
             ...
             sza=sza(good)
             rad=rad(good)
             rad_err=rad_err(good)
             iwc=iwc(good)
             iwc_err=iwc_err(good)
...
;
    if setplot eq 'ps' then begin
       ...
       device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
              /bold,/color,bits_per_pixel=8,/helvetica,filename='scatter_daily_cipsl2a_iwcunc_v5.0_vs_sza_'+sdate+'.ps'
       !p.charsize=1.25
       !p.thick=2
       !p.charthick=5
       !p.charthick=5
   ...
   !type=2^2+2^3
   plot,sza,iwc/iwc_err,xrange=[50,120],yrange=[-50,100],psym=3,color=0,xtitle='SZA',ytitle='IWC/UNC',charsize=2,charthick=2
   xyouts,55,80,sdate,/data,charsize=3,charthick=2,color=0
;  plots,90,-50
;  plots,90,100,/continue,color=0,thick=3

endif
oplot,sza,iwc/iwc_err,psym=8,color=col1(iorbit)
...
loadct,39
;xyouts,52,250,'ALB Range ('+strcompress(long(min(alb_all)),/r)+' to '+strcompress(long(max(alb_all)),/r)+')',/data,color=0,charsize=1.2,charthick=2
;xyouts,52,200,'ALBERR Range ('+strcompress(min(alb_err_all),/r)+' to '+strcompress(long(max(alb_err_all)),/r)+')',/data,color=0,charsize=1.2,charthick=2
;xyouts,92,-100,'RAD Range ('+strcompress(long(min(rad_all)),/r)+' to '+strcompress(long(max(rad_all)),/r)+')',/data,color=0,charsize=1.2,charthick=2
;xyouts,92,-150,'RADERR Range ('+strcompress(long(min(rad_err_all)),/r)+' to '+strcompress(long(max(rad_err_all)),/r)+')',/data,color=0,charsize=1.2,charthick=2
;xyouts,92,-200,'IWC Range ('+strcompress(long(min(iwc_all)),/r)+' to '+strcompress(long(max(iwc_all)),/r)+')',/data,color=0,charsize=1.2,charthick=2
;xyouts,92,-250,'IWCERR Range ('+strcompress(long(min(iwc_err_all)),/r)+' to '+strcompress(long(max(iwc_err_all)),/r)+')',/data,color=0,charsize=1.2,charthick=2
index=where(CLOUD_PRESENCE_MAP_ALL eq 1.)
if index(0) ne -1L then oplot,sza_all(index),iwc_all(index)/iwc_err_all(index),psym=8,color=250,symsize=0.5
loadct,0
...
; Close PostScript file and return control to X-windows
     ...
     if setplot eq 'ps' then begin
        device, /close
        spawn,'convert -trim scatter_daily_cipsl2a_iwcunc_v5.0_vs_sza_'+sdate+'.ps -rotate -90 scatter_daily_cipsl2a_iwcunc_v5.0_vs_sza_'+sdate+'.jpg'
        spawn,'rm -f scatter_daily_cipsl2a_iwcunc_v5.0_vs_sza_'+sdate+'.ps'
     endif
