...
dirh='/aura2/harvey/Airmass/LIMS/'
hfile=[$
'ukmo_lims_oct_2002_forward.airm']
kfile=n_elements(hfile)
for kk=0,kfile-1 do begin
close,11
openr,11,dirh+hfile(kk),/f77
read_lims_header,11,charexp,ukmo,nmc,ecmwf,restart,rfile,istime,$
...
;
    ...
    if ntraj gt 0 then $
       read_lims_airmass,11,ntraj,xn,yn,thn,agen,x0n,y0n,th0n,t0n,z0n,p0n,$
...
; plot UKMO quantity in top panel
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='lims_o3_'+stheta+$
...
;    o3min=0.
    ...
    xyouts,.35,.8,'LIMS Ozone '+stheta,/normal,charsize=2
...
; color bar for Ozone data
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert lims_o3_'+stheta+'_'+stime+'+mark_merc.ps -rotate -90 '+$
             ' lims_o3_'+stheta+'_'+stime+'+mark_merc.jpg'
    endif
