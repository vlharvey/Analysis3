;
; read CIPS level 2a version 5.00 data and plot albedo uncertainty for all orbits each day
;
loadct,0
mcolor=!p.color
...
;
    if setplot eq 'ps' then begin
       ...
       device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
              /bold,/color,bits_per_pixel=8,/helvetica,filename='scatter_daily_cipsl2a_albunc_v5.0_vs_sza_'+sdate+'.ps'
       !p.charsize=1.25
       !p.thick=2
       !p.charthick=5
       !p.charthick=5
   ...
   !type=2^2+2^3
   plot,sza,alb/alb_err,xrange=[50,120],yrange=[-50,80],psym=3,color=0,xtitle='SZA',ytitle='Cloud Albedo/UNC (10!u-6!n str!u-1!n)',charsize=2,charthick=2
   xyouts,95,60,sdate,/data,charsize=2,charthick=2,color=0
   plots,50,0
   plots,120,0,/continue,color=0,thick=3
   plots,90,-50
   plots,90,80,/continue,color=0,thick=3
...
oplot,sza,alb/alb_err,psym=8,color=col1(iorbit)
...
loadct,39
xyouts,52,70,'ALB Range ('+strcompress(long(amin),/r)+' to '+strcompress(long(amax),/r)+')',/data,color=0,charsize=1.2,charthick=2
xyouts,52,60,'ERR Range ('+strcompress(min(alb_err_all),/r)+' to '+strcompress(long(max(alb_err_all)),/r)+')',/data,color=0,charsize=1.2,charthick=2
xyouts,52,50,'SZA Range ('+strcompress(long(smin),/r)+' to '+strcompress(long(smax),/r)+')',/data,color=0,charsize=1.2,charthick=2
index=where(CLOUD_PRESENCE_MAP_ALL eq 1.)
if index(0) ne -1L then oplot,sza_all(index),alb_all(index)/alb_err_all(index),psym=8,color=250
loadct,0
...
; Close PostScript file and return control to X-windows
     ...
     if setplot eq 'ps' then begin
        device, /close
        spawn,'convert -trim scatter_daily_cipsl2a_albunc_v5.0_vs_sza_'+sdate+'.ps -rotate -90 scatter_daily_cipsl2a_albunc_v5.0_vs_sza_'+sdate+'.jpg'
        spawn,'rm -f scatter_daily_cipsl2a_albunc_v5.0_vs_sza_'+sdate+'.ps'
     endif
