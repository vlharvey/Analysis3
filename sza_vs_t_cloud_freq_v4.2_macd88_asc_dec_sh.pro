...
;
plotit:
;restore,'sza_vs_t_20071001-20071115_QF0_ALB0.sav'	;,asc_freq,des_freq,dfs,szabin,syear
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='sza_vs_t_cloud_freq_v4.2_'+syear+'_sh.ps'
   !p.charsize=1.5
   !p.thick=1.5
   !p.charthick=5
   !p.charthick=5
...
contour,asc_freq,dfs,szabin,c_color=col1,/fill,xtitle='SH DFS '+syear,ytitle='SZA',title='Ascending Node',xrange=[min(dfs),max(dfs)],yrange=[40,95],/noeras,levels=level,color=0
...
ymx=yorig(1)+ylen
set_viewport,xmn,xmx,ymn,ymx
contour,des_freq,dfs,szabin,c_color=col1,/fill,xtitle='SH DFS '+syear,ytitle='SZA',title='Descending Node',xrange=[min(dfs),max(dfs)],yrange=[40,95],/noeras,levels=level,color=0
...
; Close PostScript file and return control to X-windows
 ...
 if setplot eq 'ps' then begin
    device, /close
    spawn,'convert -trim sza_vs_t_cloud_freq_v4.2_'+syear+'_sh.ps -rotate -90 '+$
                        'sza_vs_t_cloud_freq_v4.2_'+syear+'_sh.jpg'
;   spawn,'rm -f sza_vs_t_cloud_freq_v4.2_'+syear+'_sh.ps'
 endif
save,file='sza_vs_t_20071001-20071115_QF0_ALB0.sav',asc_freq,des_freq,dfs,szabin,syear
end
