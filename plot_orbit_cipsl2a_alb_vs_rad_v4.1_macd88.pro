;
; run on MacD88
; read CIPS level 2a version 4.01 data and plot albedo orbit by orbit. x-axis is radius
;
loadct,39
mcolor=!p.color
...
if iorbit eq 0L then begin
   if setplot eq 'ps' then begin
      ...
      device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
             /bold,/color,bits_per_pixel=8,/helvetica,filename='plot_orbit_cipsl2_alb_vs_rad_v4.1_'+sdate+'.ps'
      !p.charsize=1
      !p.thick=1.5
      !p.charthick=5
      !p.charthick=5
...
plot,rad,alb,psym=3,color=0,xtitle='Radius',ytitle=ylab,title='Orbit '+sorbit,xrange=[0.,100.],yrange=[0.,100.]
...
if cloud(0) ne -1L then oplot,rad(cloud),alb(cloud),psym=3,color=mcolor*.9
...
; Close PostScript file and return control to X-windows
     ...
     if setplot eq 'ps' then begin
        device, /close
        spawn,'convert -trim plot_orbit_cipsl2_alb_vs_rad_v4.1_'+sdate+'.ps -rotate -90 '+$
                            'plot_orbit_cipsl2_alb_vs_rad_v4.1_'+sdate+'.jpg'
        spawn,'/usr/bin/rm plot_orbit_cipsl2_alb_vs_rad_v4.1_'+sdate+'.ps'
     endif
