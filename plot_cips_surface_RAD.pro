...
; color albedo surface by particle radius
; VLH 10/18/2011
;
@stddat
@kgmt
...
;
    if setplot eq 'ps' then begin
       ...
       device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
              /bold,/color,bits_per_pixel=8,filename='plot_cips_surface_'+syr+'_RAD.ps'
       !p.charsize=1.25
       !p.thick=2
       !p.charthick=2
       !p.charthick=2
...
shade_surf,smooth(alb,3),dfs,latlo,xtitle='DFS 20'+syr,ytitle='PM     Latitude     AM',color=0,ztitle='CIPS Albedo (Garys)',charsize=3,ytickv=latlo(index),$
        ...
        zticks=3,ztickv=[10,20,30,40],zrange=[0,40],/save,shades=bytscl(smooth(rad,3),tmin,tmax),pixels=result(4),xrange=[-51.,71.]
set_viewport,xmn,xmx-0.1,ymn-cbaryoff,ymn-cbaryoff+cbarydel
!type=2^2+2^3+2^6
plot,[tmin,tmax],[0,10],yrange=[0,10],xrange=[tmin,tmax],/noeras,color=0,charsize=1.5,xtitle='CIPS Radius (nm)'
ybox=[0,10,10,0,0]
x2=tmin
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert plot_cips_surface_'+syr+'_RAD.ps -rotate -90 plot_cips_surface_'+syr+'_RAD.jpg'
;  spawn,'rm -f plot_cips_surface_'+syr+'_RAD.ps'
endif
end
