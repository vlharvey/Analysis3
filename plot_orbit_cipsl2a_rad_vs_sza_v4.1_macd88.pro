;
; run on MacD88
; read CIPS level 2a version 4.01 data and plot radius orbit by orbit
;
loadct,39
mcolor=!p.color
...
if iorbit eq 0L then begin
   if setplot eq 'ps' then begin
      ...
      device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
             /bold,/color,bits_per_pixel=8,/helvetica,filename='plot_orbit_cipsl2_rad_vs_sza_v4.1_'+sdate+'.ps'
      !p.charsize=1
      !p.thick=1.5
      !p.charthick=5
      !p.charthick=5
...
ylab=' '
if iorbit eq 0 or iorbit eq 5 or iorbit eq 10 then ylab='Radius'
plot,sza,rad,psym=3,color=0,xtitle='SZA',ytitle=ylab,title='Orbit '+sorbit,xrange=[40.,90.],yrange=[0.,100.]
...
if cloud(0) ne -1L then oplot,sza(cloud),rad(cloud),psym=3,color=mcolor*.9
print,'RAD bounds ',min(rad),max(rad)
...
; Close PostScript file and return control to X-windows
     ...
     if setplot eq 'ps' then begin
        device, /close
        spawn,'convert -trim plot_orbit_cipsl2_rad_vs_sza_v4.1_'+sdate+'.ps -rotate -90 '+$
                            'plot_orbit_cipsl2_rad_vs_sza_v4.1_'+sdate+'.jpg'
        spawn,'/usr/bin/rm plot_orbit_cipsl2_rad_vs_sza_v4.1_'+sdate+'.ps'
     endif
