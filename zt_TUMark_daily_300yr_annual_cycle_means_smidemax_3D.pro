...
;
    if setplot eq 'ps' then begin
       ...
       device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
              /bold,/color,bits_per_pixel=8,/helvetica,filename='zt_TUMark_NH_SH_smidemax.ps'
       !p.charsize=1.25
       !p.thick=2
       !p.charthick=2
       !y.thick=2
...
;
    ...
    contour,tnh,findgen(ndays),znh,/noera,/fill,color=0,c_color=col1,levels=tlevel,xrange=[0,ndays-1],yrange=[30,125],ytitle='Altitude (km)',charsize=1.5,charthick=2,title='NH',$
            xticks=nxticks-1,xtickname=xlabs,xtickv=xindex
    contour,tnh,findgen(ndays),znh,/noera,/foll,color=0,thick=2,levels=5*findgen(20),/overplot
mnh=smooth(mnh,3,/edge_truncate)
    ...
    contour,tsh,findgen(ndays),zsh,/noera,/fill,color=0,c_color=col1,levels=tlevel,xrange=[0,ndays-1],yrange=[30,125],ytitle='Altitude (km)',charsize=1.5,charthick=2,title='SH',$
            xticks=nxticks-1,xtickname=xlabs,xtickv=xindex
    contour,tsh,findgen(ndays),zsh,/noera,/foll,color=0,thick=2,levels=5*findgen(20),/overplot
...
;
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim zt_TUMark_NH_SH_smidemax.ps -rotate -90 zt_TUMark_NH_SH_smidemax.jpg
;      spawn,'rm -f zt_TUMark_NH_SH_smidemax.ps'
    endif
