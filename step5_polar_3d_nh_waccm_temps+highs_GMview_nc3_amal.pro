...
lstmn=10L & lstdy=1L & lstyr=1996L
ledmn=3L & leddy=30L & ledyr=1997L
lstday=0L & ledday=0L
;
; get date range
;
...
;   if l eq 0 then begin
       zindex=where(th ge 600. and th le 20000.,nth2)
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='polar_3d_nh_waccm_'+sdate+'_GM+highs_nc3_amal.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !p.charthick=5
...
col1=1+(findgen(nlvls)/nlvls)*mcolor
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim polar_3d_nh_waccm_'+sdate+'_GM+highs_nc3_amal.ps -rotate -90 '+$
                           'polar_3d_nh_waccm_'+sdate+'_GM+highs_nc3_amal.jpg'
;      spawn,'rm -f polar_3d_nh_waccm_'+sdate+'_GM+highs_nc3_amal.ps'
    endif
goto, jump
end
