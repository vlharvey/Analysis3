;
; save Line_Plots/F_V4.20_r05_80Lat_2G_all_sh_plot_data.sav
; save Line_plots/F_V4.20_r05_80Lat_2G_all_nh_plot_data.sav
;
; that contain:
;
...
DEVICE,/BOLD
DEVICE, FILENAME=ptho+'F_V'+ver+'_'+slat+'Lat_'+G+'G_2hem.ps'
endif
...
seasons_sh=['SH07','SH08','SH09','SH10','SH11','SH12','SH13','SH14','SH15','SH16']
if hem eq 'NH' then begin
   seasons=seasons_nh
   shem='N'
   jdaysol=172.
   col1=[0,20,40,60,70,80,90,100,110,120]
   outfile=ptho+'F_V4.20_'+slat+'Lat_'+G+'G_all_nh_plot_data.sav'
endif else begin
   seasons=seasons_sh
   shem='S'
   jdaysol=355.
   col1=[150,160,170,180,190,200,210,220,230,0]
   outfile=ptho+'F_V4.20_'+slat+'Lat_'+G+'G_all_sh_plot_data.sav'
endelse
...
if isea eq 0L then begin
   PLOT_F_ASC=fltarr(130,nseasons)
   PLOT_F_DSC=fltarr(130,nseasons)
...
PLOT_F_ASC(0:ndays-1,isea)=smooth(100.*array(*,ilat(0)),3,/nan)      
PLOT_F_DSC(0:ndays-1,isea)=smooth(100.*array(*,ilat(1)),3,/nan)
...
;  cols=[0,11,3,1]
   ...
   oplot,ddd,smooth(100.*array(*,ilat),3,/nan),thick=10,color=col1(isea)
...
save,file=ptho+'F_V'+ver+'_'+slat+'Lat_'+G+'G_'+season+'.sav',ddd,freq
ENDFOR
; ************ END LOOP OVER SEASONS ******************
;
; save all seasons in one file
;
save,file=outfile,PLOT_DAYS_ASC,PLOT_DAYS_DSC,PLOT_F_ASC,PLOT_F_DSC,SEASONS,SLAT
