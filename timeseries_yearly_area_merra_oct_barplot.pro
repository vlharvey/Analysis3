...
;read,'Enter desired theta ',rth
          ...
          if setplot eq 'ps' then begin
             ...
             !p.font=0
             device,font_size=9
             device,/landscape,bits=8,filename='barplot_area_merra_'+sth+'_oct.ps'
             device,/color
...
plot,lyear,area_allyears,yrange=[0,45],color=0,charsize=2,charthick=2,ytitle=sth+' Vortex Area (%)',xrange=[min(lyear),max(lyear)],psym=8,/noeras,symsize=1.5
...
;oplot,lyear,arrmean-arrsig+0*lyear,color=0,thick=1
xyouts,xmn+0.01,ymx-0.03,'October',/normal,charsize=2,charthick=2,color=0
;
; solar cycle?
;
...
if setplot eq 'ps' then begin
   device,/close
   spawn,'convert -trim barplot_area_merra_'+sth+'_oct.ps -rotate -90 barplot_area_merra_'+sth+'_oct.jpg'
endif
