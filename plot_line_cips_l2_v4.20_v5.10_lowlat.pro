...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='line_cips_l2_v4.20_v5.10_medians_allyears_lowlat.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !y.thick=2
...
; ALB
;
restore,'cips_l2_medians_vs_sza_north_2007_lowlat.sav
;print,szabin
;rsza=70.
...
xyouts,.35,.95,'Lats Equatorward of 65',/normal,color=0,charsize=2,charthick=2
set_viewport,xorig(0),xorig(0)+xlen,yorig(0),yorig(0)+ylen
dum42=mean(medalb20_all,dim=2,/nan)
dum51=mean(medalb51_all,dim=2,/nan)
plot,dfs_all,dum42-dum51,xrange=[-20,40],yrange=[-5,5],ytitle='ALB v4 - v5',charsize=1.25,charthick=2,/noeras,color=col1(0),psym=8,title='North'
...
for iyear=2008,2015 do begin            ; 2015/2016 is the last SH v4.2 season
    ...
    dum42=mean(medalb20_all,dim=2,/nan)
    dum51=mean(medalb51_all,dim=2,/nan)
    oplot,dfs_all,dum42-dum51,psym=8,color=col1(iyear-2007)
...
dum42=mean(mediwc20_all,dim=2,/nan)
dum51=mean(mediwc51_all,dim=2,/nan)
plot,dfs_all,dum42-dum51,xrange=[-20,40],yrange=[-50,50],ytitle='IWC v4 - v5',charsize=1.25,charthick=2,/noeras,color=col1(0),psym=8
oplot,dfs_all,0*dfs_all,linestyle=5,color=col1(0)
for iyear=2008,2015 do begin            ; 2015/2016 is the last SH v4.2 season
    syear=strcompress(long(iyear),/r)
    restore,'cips_l2_medians_vs_sza_north_'+syear+'_lowlat.sav
...
dum42=mean(medrad20_all,dim=2,/nan)
dum51=mean(medrad51_all,dim=2,/nan)
plot,dfs_all,dum42-dum51,xrange=[-20,40],yrange=[-50,50],ytitle='RAD v4 - v5',charsize=1.25,charthick=2,/noeras,color=col1(0),psym=8,xtitle='DFS'
oplot,dfs_all,0*dfs_all,linestyle=5,color=col1(0)
for iyear=2008,2015 do begin            ; 2015/2016 is the last SH v4.2 season
    syear=strcompress(long(iyear),/r)
    restore,'cips_l2_medians_vs_sza_north_'+syear+'_lowlat.sav
...
; ALB
;
restore,'cips_l2_medians_vs_sza_south_2007_lowlat.sav
npts=100.
index=where(num20_all lt npts)
if index(0) ne -1L then medalb20_all(index)=0./0.
index=where(num51_all lt npts)
...
dum42=mean(medalb20_all,dim=2,/nan)
dum51=mean(medalb51_all,dim=2,/nan)
plot,dfs_all,dum42-dum51,xrange=[-20,40],yrange=[-5,5],ytitle='ALB v4 - v5',charsize=1.25,charthick=2,/noeras,color=col1(0),psym=8,title='South'
oplot,dfs_all,0*dfs_all,linestyle=5,color=col1(0)
;xyouts,xorig(3)+xlen+0.02,yorig(3)+ylen-0.02,'2007',/normal,color=0,charthick=2
for iyear=2008,2013 do begin            ; 2015/2016 is the last SH v4.2 season
...
for iyear=2008,2013 do begin            ; 2015/2016 is the last SH v4.2 season
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim line_cips_l2_v4.20_v5.10_medians_allyears_lowlat.ps -rotate -90 line_cips_l2_v4.20_v5.10_medians_allyears_lowlat.png'
    endif
end
