;
; mercator plots of SD-WACCM water and MLS water 
; 
; LAT             DOUBLE    = Array[96]
; LEV             DOUBLE    = Array[88]
...
;
      if setplot eq 'ps' then begin
         ...
         !p.font=0
         device,font_size=9
         device,/landscape,bits=8,filename='merc_sdwaccm+mls_h2o_'+sdate+'_'+slev+'.ps'
         device,/color
    ...
    contour,pp2,alon,alat,levels=14+0.2*findgen(21),/follow,color=mcolor,/noeras,/overplot,thick=3
imin=min(level)
imax=max(level)
...
contour,gplev,lon,lat,levels=14+0.2*findgen(21),/follow,color=mcolor,/noeras,/overplot,thick=3
imin=min(level)
imax=max(level)
...
;
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim merc_sdwaccm+mls_h2o_'+sdate+'_'+slev+'.ps -rotate -90 merc_sdwaccm+mls_h2o_'+sdate+'_'+slev+'.jpg'
    endif
